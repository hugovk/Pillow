...
cms_profile_fromstring(PyObject *self, PyObject *args)
{
    ...
    char *pProfile;
    Py_ssize_t nProfile;
    if (!PyArg_ParseTuple(args, "y#:profile_frombytes", &pProfile,
                          &nProfile)) {
        return NULL;
    }
...
pyCMScopyAux(cmsHTRANSFORM hTransform, Imaging imDst, const Imaging imSrc)
{
    ...
    for (e = 0; e < numExtras; ++e) {
        ...
        for (y = 0; y < ySize; y++) {
            ...
            const char *pSrcExtras =
                imSrc->image[y] + srcChannel * channelSize;
...
pyCMSdoTransform(Imaging im, Imaging imOut, cmsHTRANSFORM hTransform)
{
    ...
    // do the same thing automagically. Unfortunately, lcms2.8 is not yet
    // widely enough available on all platforms, so we polyfill it here for
    // now.
...
_buildTransform(cmsHPROFILE hInputProfile, cmsHPROFILE hOutputProfile,
                char *sInMode, char *sOutMode, int iRenderingIntent,
...
buildTransform(PyObject *self, PyObject *args)
{
    ...
    transform =
        _buildTransform(pInputProfile->profile, pOutputProfile->profile,
                        sInMode, sOutMode, iRenderingIntent, cmsFLAGS);
...
buildProofTransform(PyObject *self, PyObject *args)
{
    ...
    transform =
        _buildProofTransform(pInputProfile->profile, pOutputProfile->profile,
                             pProofProfile->profile, sInMode, sOutMode,
...
createProfile(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "s|d:createProfile", &sColorSpace,
                          &dColorTemp)) {
        return NULL;
    }
...
cms_profile_is_intent_supported(CmsProfileObject *self, PyObject *args)
{
    ...
    int intent;
    int direction;
    if (!PyArg_ParseTuple(args, "ii:is_intent_supported", &intent,
                          &direction)) {
        return NULL;
    }
...
static cmsBool
_check_intent(int clut, cmsHPROFILE hProfile, cmsUInt32Number Intent,
...
{
    PyErr_WarnEx(PyExc_DeprecationWarning,
                 "product_manufacturer is deprecated. Use Unicode "
...
{
    return _profile_read_int_as_string(
        cmsGetHeaderManufacturer(self->profile));
}
                                              ...
                                              void *closure)
{
    ...
    return Py_BuildValue("{s:i,s:(ddd),s:s,s:d,s:s}", "observer", mc->Observer,
                         "backing", mc->Backing.X, mc->Backing.Y,
                         mc->Backing.Z, "geo", geo, "flare", mc->Flare,

...
PyObject* PyImagingNew(Imaging imOut) {
...
static void _dealloc(ImagingObject* imagep) {
    ...
    if (imagep->access) ImagingAccessDelete(imagep->image, imagep->access);
...
void* ImagingError_MemoryError(void) { return PyErr_NoMemory(); }
...
void ImagingError_Clear(void) { PyErr_Clear(); }

/* -------------------------------------------------------------------- */
/* HELPERS                                */
/* -------------------------------------------------------------------- */

static int getbands(const char* mode) {
    Imaging im;
    int bands;
...
static void* getlist(PyObject* arg, Py_ssize_t* length,
                     const char* wrong_length, int type) {
...
static inline PyObject* getpixel(Imaging im, ImagingAccess access, int x,
                                 int y) {
    union {
        UINT8 b[4];
        UINT16 h;
    ...
    switch (im->type) {
        case IMAGING_TYPE_UINT8:
            switch (im->bands) {
                ...
                case 3:
                    return Py_BuildValue("BBB", pixel.b[0], pixel.b[1],
                                         pixel.b[2]);
                case 4:
                    return Py_BuildValue("BBBB", pixel.b[0], pixel.b[1],
                                         pixel.b[2], pixel.b[3]);
            }
            break;
...
static char* getink(PyObject* color, Imaging im, char* ink) {
    int g = 0, b = 0, a = 0;
    double f = 0;
...
static PyObject* _new(PyObject* self, PyObject* args) {
    ...
    if (!PyArg_ParseTuple(args, "s(ii)", &mode, &xsize, &ysize)) return NULL;

    return PyImagingNew(ImagingNew(mode, xsize, ysize));
}

static PyObject* _new_block(PyObject* self, PyObject* args) {
    char* mode;
    int xsize, ysize;
...
static PyObject* _linear_gradient(PyObject* self, PyObject* args) {
    char* mode;
...
static PyObject* _radial_gradient(PyObject* self, PyObject* args) {
    char* mode;
...
static PyObject* _alpha_composite(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep1;
    ImagingObject* imagep2;

    if (!PyArg_ParseTuple(args, "O!O!", &Imaging_Type, &imagep1, &Imaging_Type,
                          &imagep2))
        return NULL;
...
static PyObject* _blend(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep1;
    ImagingObject* imagep2;
    double alpha;

    alpha = 0.5;
    if (!PyArg_ParseTuple(args, "O!O!|d", &Imaging_Type, &imagep1,
                          &Imaging_Type, &imagep2, &alpha))
        return NULL;

    return PyImagingNew(
        ImagingBlend(imagep1->image, imagep2->image, (float)alpha));
}
...
static INT16* _prepare_lut_table(PyObject* table, Py_ssize_t table_size) {
    int i;
...
#define PRECISION_BITS (16 - 8 - 2)
    ...
    if (!prepared) {
        if (free_table_data) free(table_data);
    ...
    for (i = 0; i < table_size; i++) {
        ...
        switch (data_type) {
            case TYPE_FLOAT16:
                ...
                break;
            case TYPE_FLOAT32:
                memcpy(&item, ((char*)table_data) + i * sizeof(FLOAT32),
                       sizeof(FLOAT32));
                break;
            case TYPE_DOUBLE:
                memcpy(&dtmp, ((char*)table_data) + i * sizeof(dtmp),
                       sizeof(dtmp));
                item = (FLOAT32)dtmp;
                break;
        }
...
static PyObject* _color_lut_3d(ImagingObject* self, PyObject* args) {
    char* mode;
    int filter;
    int table_channels;
    ...
    if (!PyArg_ParseTuple(args, "siiiiiO:color_lut_3d", &mode, &filter,
                          &table_channels, &size1D, &size2D, &size3D, &table)) {
        return NULL;
    }
    ...
    if (!ImagingColorLUT3D_linear(imOut, self->image, table_channels, size1D,
                                  size2D, size3D, prepared_table)) {
        free(prepared_table);
        ImagingDelete(imOut);
        return NULL;
    }
...
static PyObject* _convert(ImagingObject* self, PyObject* args) {
    char* mode;
    int dither = 0;
    ...
    if (paletteimage != NULL) {
        if (!PyImaging_Check(paletteimage)) {
            ...
            PyErr_SetString(PyExc_ValueError,
                            "palette argument must be image with mode 'P'");
            return NULL;
        }
    ...
    return PyImagingNew(ImagingConvert(
        self->image, mode, paletteimage ? paletteimage->image->palette : NULL,
        dither));
}

static PyObject* _convert2(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep1;
    ImagingObject* imagep2;
    if (!PyArg_ParseTuple(args, "O!O!", &Imaging_Type, &imagep1, &Imaging_Type,
                          &imagep2))
    ...
    if (!ImagingConvert2(imagep1->image, imagep2->image)) return NULL;

    Py_INCREF(Py_None);
    return Py_None;
}

static PyObject* _convert_matrix(ImagingObject* self, PyObject* args) {
    char* mode;
    float m[12];
    if (!PyArg_ParseTuple(args, "s(ffff)", &mode, m + 0, m + 1, m + 2, m + 3)) {
        PyErr_Clear();
        if (!PyArg_ParseTuple(args, "s(ffffffffffff)", &mode, m + 0, m + 1,
...
static PyObject* _convert_transparent(ImagingObject* self, PyObject* args) {
    char* mode;
    int r, g, b;
    if (PyArg_ParseTuple(args, "s(iii)", &mode, &r, &g, &b)) {
        return PyImagingNew(
            ImagingConvertTransparent(self->image, mode, r, g, b));
    }
    PyErr_Clear();
    if (PyArg_ParseTuple(args, "si", &mode, &r)) {
        return PyImagingNew(
            ImagingConvertTransparent(self->image, mode, r, 0, 0));
    }
    return NULL;
}

static PyObject* _copy(ImagingObject* self, PyObject* args) {
    if (!PyArg_ParseTuple(args, "")) return NULL;

    return PyImagingNew(ImagingCopy(self->image));
}

static PyObject* _crop(ImagingObject* self, PyObject* args) {
    int x0, y0, x1, y1;
...
static PyObject* _expand_image(ImagingObject* self, PyObject* args) {
    int x, y;
    int mode = 0;
    if (!PyArg_ParseTuple(args, "ii|i", &x, &y, &mode)) return NULL;

    return PyImagingNew(ImagingExpand(self->image, x, y, mode));
}

static PyObject* _filter(ImagingObject* self, PyObject* args) {
    PyObject* imOut;
    ...
    if (!PyArg_ParseTuple(args, "(ii)ffO", &xsize, &ysize, &divisor, &offset,
                          &kernel))
        return NULL;
...
static PyObject* _gaussian_blur(ImagingObject* self, PyObject* args) {
    Imaging imIn;
    Imaging imOut;

    float radius = 0;
    int passes = 3;
    if (!PyArg_ParseTuple(args, "f|i", &radius, &passes)) return NULL;
...
static PyObject* _getpalette(ImagingObject* self, PyObject* args) {
    PyObject* palette;
    int palettesize = 256;
    int bits;
    ...
    char* mode = "RGB";
    char* rawmode = "RGB";
    if (!PyArg_ParseTuple(args, "|ss", &mode, &rawmode)) return NULL;
    ...
    pack((UINT8*)PyBytes_AsString(palette), self->image->palette->palette,
         palettesize);

    return palette;
}

static PyObject* _getpalettemode(ImagingObject* self, PyObject* args) {
    if (!self->image->palette) {
        PyErr_SetString(PyExc_ValueError, no_palette);
        return NULL;
    }
...
static inline int _getxy(PyObject* xy, int* x, int* y) {
    PyObject* value;

    if (!PyTuple_Check(xy) || PyTuple_GET_SIZE(xy) != 2) goto badarg;
...
static PyObject* _getpixel(ImagingObject* self, PyObject* args) {
    PyObject* xy;
    int x, y;
...
static union hist_extrema* parse_histogram_extremap(ImagingObject* self,
                                                    PyObject* extremap,
    ...
    if (extremap) {
        switch (self->image->type) {
            case IMAGING_TYPE_UINT8:
                if (!PyArg_ParseTuple(extremap, "ii", &i0, &i1)) return NULL;
                ep->u[0] = CLIP8(i0);
                ep->u[1] = CLIP8(i1);
                break;
            case IMAGING_TYPE_INT32:
                ...
                break;
            case IMAGING_TYPE_FLOAT32:
                if (!PyArg_ParseTuple(extremap, "dd", &f0, &f1)) return NULL;
                ep->f[0] = (FLOAT32)f0;
                ep->f[1] = (FLOAT32)f1;
                break;
            default:
...
static PyObject* _histogram(ImagingObject* self, PyObject* args) {
    ImagingHistogram h;
    PyObject* list;
    int i;
...
static PyObject* _entropy(ImagingObject* self, PyObject* args) {
    ImagingHistogram h;
    int idx, length;
    long sum;
...
static PyObject* _modefilter(ImagingObject* self, PyObject* args) {
    int size;
...
static PyObject* _offset(ImagingObject* self, PyObject* args) {
    int xoffset, yoffset;
    if (!PyArg_ParseTuple(args, "ii", &xoffset, &yoffset)) return NULL;

    return PyImagingNew(ImagingOffset(self->image, xoffset, yoffset));
}

static PyObject* _paste(ImagingObject* self, PyObject* args) {
    int status;
    char ink[4];
    ...
    int x0, y0, x1, y1;
    ImagingObject* maskp = NULL;
    if (!PyArg_ParseTuple(args, "O(iiii)|O!", &source, &x0, &y0, &x1, &y1,
                          &Imaging_Type, &maskp))
        return NULL;
    ...
    if (PyImaging_Check(source))
        ...
        if (!getink(source, self->image, ink)) return NULL;
        status = ImagingFill2(self->image, ink, (maskp) ? maskp->image : NULL,
                              x0, y0, x1, y1);
    }
...
static PyObject* _point(ImagingObject* self, PyObject* args) {
    ...
    PyObject* list;
    char* mode;
    if (!PyArg_ParseTuple(args, "Oz", &list, &mode)) return NULL;
    ...
    if (mode && !strcmp(mode, "F")) {
        ...
        if (mode && !strcmp(mode, "I"))
            ...
            for (i = 0; i < 256; i++) {
                ...
                if (n > 768) lut[i * 4 + 3] = CLIP8(data[i + 768]);
            }
            im = ImagingPoint(self->image, mode, (void*)lut);
        } else {
            /* map individual bands */
            for (i = 0; i < n; i++) lut[i] = CLIP8(data[i]);
            im = ImagingPoint(self->image, mode, (void*)lut);
        }
        free(data);
    }
...
static PyObject* _point_transform(ImagingObject* self, PyObject* args) {
    double scale = 1.0;
    double offset = 0.0;
    if (!PyArg_ParseTuple(args, "|dd", &scale, &offset)) return NULL;

    return PyImagingNew(ImagingPointTransform(self->image, scale, offset));
}

static PyObject* _putdata(ImagingObject* self, PyObject* args) {
    Imaging image;
    // i & n are # pixels, require py_ssize_t. x can be as large as n. y, just
    // because.
    Py_ssize_t n, i, x, y;
    ...
    if (!PyArg_ParseTuple(args, "O|dd", &data, &scale, &offset)) return NULL;
    ...
    if (image->image8) {
        if (PyBytes_Check(data)) {
            ...
            if (scale == 1.0 && offset == 0.0) /* Plain string data */
                for (i = y = 0; i < n; i += image->xsize, y++) {
                    x = n - i;
                    if (x > (int)image->xsize) x = image->xsize;
...
static PyObject* _quantize(ImagingObject* self, PyObject* args) {
    int colours = 256;
    int method = 0;
    int kmeans = 0;
...
static PyObject* _putpalette(ImagingObject* self, PyObject* args) {
...
static PyObject* _putpalettealpha(ImagingObject* self, PyObject* args) {
    int index;
    int alpha = 0;
    if (!PyArg_ParseTuple(args, "i|i", &index, &alpha)) return NULL;
...
static PyObject* _putpalettealphas(ImagingObject* self, PyObject* args) {
    int i;
    UINT8* values;
    Py_ssize_t length;
    if (!PyArg_ParseTuple(args, "y#", &values, &length)) return NULL;
...
static PyObject* _putpixel(ImagingObject* self, PyObject* args) {
    Imaging im;
    char ink[4];

    int x, y;
    PyObject* color;
    if (!PyArg_ParseTuple(args, "(ii)O", &x, &y, &color)) return NULL;
    ...
    if (!getink(color, im, ink)) return NULL;

    if (self->access) self->access->put_pixel(im, x, y, ink);
...
static PyObject* _rankfilter(ImagingObject* self, PyObject* args) {
    int size, rank;
    if (!PyArg_ParseTuple(args, "ii", &size, &rank)) return NULL;
...
static PyObject* _resize(ImagingObject* self, PyObject* args) {
    ...
    if (box[0] - (int)box[0] == 0 && box[2] - box[0] == xsize &&
        ...
        imOut = ImagingTransform(imOut, imIn, IMAGING_TRANSFORM_AFFINE, 0, 0,
                                 xsize, ysize, a, filter, 1);
    } else {
...
static PyObject* _reduce(ImagingObject* self, PyObject* args) {
    Imaging imIn;
    Imaging imOut;
    ...
    if (!PyArg_ParseTuple(args, "(ii)|(iiii)", &xscale, &yscale, &box[0],
                          &box[1], &box[2], &box[3]))
        return NULL;
...
static PyObject* im_setmode(ImagingObject* self, PyObject* args) {
    ...
    char* mode;
    Py_ssize_t modelen;
    if (!PyArg_ParseTuple(args, "s#:setmode", &mode, &modelen)) return NULL;
    ...
    if (!strcmp(im->mode, mode)) {
        ...
        strcpy(im->mode, mode);
        im->bands = modelen;
        if (!strcmp(mode, "RGBA")) (void)ImagingFillBand(im, 3, 255);
    } else {
        /* trying doing an in-place conversion */
        if (!ImagingConvertInPlace(im, mode)) return NULL;
    }

    if (self->access) ImagingAccessDelete(im, self->access);
...
static PyObject* _transform2(ImagingObject* self, PyObject* args) {
    ...
    int filter = IMAGING_TRANSFORM_NEAREST;
    int fill = 1;
    if (!PyArg_ParseTuple(args, "(iiii)O!iO|ii", &x0, &y0, &x1, &y1,
                          &Imaging_Type, &imagep, &method, &data, &filter,
                          &fill))
        return NULL;
    ...
    imOut = ImagingTransform(self->image, imagep->image, method, x0, y0, x1, y1,
                             a, filter, fill);
...
static PyObject* _transpose(ImagingObject* self, PyObject* args) {
    Imaging imIn;
    Imaging imOut;

    int op;
    if (!PyArg_ParseTuple(args, "i", &op)) return NULL;
...
#ifdef WITH_UNSHARPMASK
static PyObject* _unsharp_mask(ImagingObject* self, PyObject* args) {
    Imaging imIn;
    Imaging imOut;
...
static PyObject* _box_blur(ImagingObject* self, PyObject* args) {
    Imaging imIn;
    Imaging imOut;

    float radius;
    int n = 1;
    if (!PyArg_ParseTuple(args, "f|i", &radius, &n)) return NULL;
...
static PyObject* _isblock(ImagingObject* self, PyObject* args) {
    return PyBool_FromLong(self->image->block != NULL);
}

static PyObject* _getbbox(ImagingObject* self, PyObject* args) {
    int bbox[4];
...
static PyObject* _getcolors(ImagingObject* self, PyObject* args) {
    ImagingColorItem* items;
    int i, colors;
    PyObject* out;

    int maxcolors = 256;
    if (!PyArg_ParseTuple(args, "i:getcolors", &maxcolors)) return NULL;
    ...
    if (colors > maxcolors) {
        ...
        for (i = 0; i < colors; i++) {
            ...
            PyObject* item =
                Py_BuildValue("iN", v->count,
                              getpixel(self->image, self->access, v->x, v->y));
...
static PyObject* _getextrema(ImagingObject* self, PyObject* args) {
    union {
        UINT8 u[2];
        INT32 i[2];
        FLOAT32 f[2];
    ...
    if (status) switch (self->image->type) {
            case IMAGING_TYPE_UINT8:
...
static PyObject* _getprojection(ImagingObject* self, PyObject* args) {
    ...
    result = Py_BuildValue("y#y#", xprofile, (Py_ssize_t)self->image->xsize,
...
static PyObject* _getband(ImagingObject* self, PyObject* args) {
    int band;

    if (!PyArg_ParseTuple(args, "i", &band)) return NULL;

    return PyImagingNew(ImagingGetBand(self->image, band));
}

static PyObject* _fillband(ImagingObject* self, PyObject* args) {
    int band;
    int color;

    if (!PyArg_ParseTuple(args, "ii", &band, &color)) return NULL;

    if (!ImagingFillBand(self->image, band, color)) return NULL;

    Py_INCREF(Py_None);
    return Py_None;
}

static PyObject* _putband(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep;
    int band;
    if (!PyArg_ParseTuple(args, "O!i", &Imaging_Type, &imagep, &band))
        return NULL;

    if (!ImagingPutBand(self->image, imagep->image, band)) return NULL;

    Py_INCREF(Py_None);
    return Py_None;
}

static PyObject* _merge(PyObject* self, PyObject* args) {
    char* mode;
    ...
    if (!PyArg_ParseTuple(args, "sO!|O!O!O!", &mode, &Imaging_Type, &band0,
                          &Imaging_Type, &band1, &Imaging_Type, &band2,
                          &Imaging_Type, &band3))
        return NULL;
...
static PyObject* _split(ImagingObject* self, PyObject* args) {
    int fails = 0;
    Py_ssize_t i;
    PyObject* list;
...
static PyObject* _chop_invert(ImagingObject* self, PyObject* args) {
    return PyImagingNew(ImagingNegative(self->image));
}

static PyObject* _chop_lighter(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep;

    if (!PyArg_ParseTuple(args, "O!", &Imaging_Type, &imagep)) return NULL;

    return PyImagingNew(ImagingChopLighter(self->image, imagep->image));
}

static PyObject* _chop_darker(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep;
...
static PyObject* _chop_difference(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep;
...
static PyObject* _chop_multiply(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep;
...
static PyObject* _chop_screen(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep;
...
static PyObject* _chop_add(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep;
    float scale;
    int offset;
    ...
    if (!PyArg_ParseTuple(args, "O!|fi", &Imaging_Type, &imagep, &scale,
                          &offset))
        return NULL;

    return PyImagingNew(
        ImagingChopAdd(self->image, imagep->image, scale, offset));
}

static PyObject* _chop_subtract(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep;
    float scale;
    int offset;
    ...
    return PyImagingNew(
        ImagingChopSubtract(self->image, imagep->image, scale, offset));
}

static PyObject* _chop_and(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep;
...
static PyObject* _chop_or(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep;
...
static PyObject* _chop_xor(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep;
...
static PyObject* _chop_add_modulo(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep;
...
static PyObject* _chop_subtract_modulo(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep;
...
static PyObject* _chop_soft_light(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep;
...
static PyObject* _chop_hard_light(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep;
...
static PyObject* _chop_overlay(ImagingObject* self, PyObject* args) {
    ImagingObject* imagep;
...
static PyObject* _font_new(PyObject* self_, PyObject* args) {
    ...
    for (i = 0; i < 256; i++) {
        ...
        if (self->glyphs[i].dy0 < y0) y0 = self->glyphs[i].dy0;
        if (self->glyphs[i].dy1 > y1) y1 = self->glyphs[i].dy1;
        glyphdata += 20;
    }
...
static void _font_dealloc(ImagingFontObject* self) {
    Py_XDECREF(self->ref);
    PyObject_Del(self);
}

static inline int textwidth(ImagingFontObject* self,
                            const unsigned char* text) {
    int xsize;

    for (xsize = 0; *text; text++) xsize += self->glyphs[*text].dx;
...
static PyObject* _font_getmask(ImagingFontObject* self, PyObject* args) {
    Imaging im;
    Imaging bitmap;
    int x, b;
    int i = 0;
    ...
    for (x = 0; text[i]; i++) {
        ...
        bitmap = ImagingCrop(self->bitmap, glyph->sx0, glyph->sy0, glyph->sx1,
                             glyph->sy1);
        if (!bitmap) goto failed;
        status = ImagingPaste(im, bitmap, NULL, glyph->dx0 + x, glyph->dy0 + b,
                              glyph->dx1 + x, glyph->dy1 + b);
...
static PyObject* _font_getsize(ImagingFontObject* self, PyObject* args) {
    unsigned char* text;
    PyObject* encoded_string;
    PyObject* val;

    if (!PyArg_ParseTuple(args, "O:getsize", &encoded_string)) return NULL;
...
static PyObject* _draw_new(PyObject* self_, PyObject* args) {
...
static void _draw_dealloc(ImagingDrawObject* self) {
...
static PyObject* _draw_ink(ImagingDrawObject* self, PyObject* args) {
    INT32 ink = 0;
    PyObject* color;
    if (!PyArg_ParseTuple(args, "O", &color)) return NULL;

    if (!getink(color, self->image->image, (char*)&ink)) return NULL;

    return PyLong_FromLong((int)ink);
}

static PyObject* _draw_arc(ImagingDrawObject* self, PyObject* args) {
    double* xy;
    Py_ssize_t n;
    ...
    n = ImagingDrawArc(self->image->image, (int)xy[0], (int)xy[1], (int)xy[2],
                       (int)xy[3], start, end, &ink, width, op);
...
static PyObject* _draw_bitmap(ImagingDrawObject* self, PyObject* args) {
    double* xy;
    Py_ssize_t n;
    ...
    n = ImagingDrawBitmap(self->image->image, (int)xy[0], (int)xy[1],
                          bitmap->image, &ink, self->blend);
...
static PyObject* _draw_chord(ImagingDrawObject* self, PyObject* args) {
    double* xy;
    Py_ssize_t n;
    ...
    int width = 0;
    float start, end;
    if (!PyArg_ParseTuple(args, "Offii|i", &data, &start, &end, &ink, &fill,
                          &width))
        return NULL;
    ...
    n = ImagingDrawChord(self->image->image, (int)xy[0], (int)xy[1], (int)xy[2],
                         (int)xy[3], start, end, &ink, fill, width,
                         self->blend);
...
static PyObject* _draw_ellipse(ImagingDrawObject* self, PyObject* args) {
    double* xy;
    Py_ssize_t n;
    ...
    n = ImagingDrawEllipse(self->image->image, (int)xy[0], (int)xy[1],
                           (int)xy[2], (int)xy[3], &ink, fill, width,
                           self->blend);
...
static PyObject* _draw_lines(ImagingDrawObject* self, PyObject* args) {
    double* xy;
    Py_ssize_t i, n;
    ...
    int ink;
    int width = 0;
    if (!PyArg_ParseTuple(args, "Oi|i", &data, &ink, &width)) return NULL;
    ...
    if (width <= 1) {
        ...
        for (i = 0; i < n - 1; i++) {
            ...
            if (ImagingDrawLine(self->image->image, (int)p[0], (int)p[1],
                                (int)p[2], (int)p[3], &ink, self->blend) < 0) {
                free(xy);
                return NULL;
            }
        }
...
static PyObject* _draw_points(ImagingDrawObject* self, PyObject* args) {
    double* xy;
    Py_ssize_t i, n;

    PyObject* data;
    int ink;
    if (!PyArg_ParseTuple(args, "Oi", &data, &ink)) return NULL;
...
static PyObject* _draw_outline(ImagingDrawObject* self, PyObject* args) {
    ImagingOutline outline;

    PyObject* outline_;
    int ink;
    int fill = 0;
    if (!PyArg_ParseTuple(args, "Oi|i", &outline_, &ink, &fill)) return NULL;
    ...
    if (ImagingDrawOutline(self->image->image, outline, &ink, fill,
                           self->blend) < 0)
        return NULL;
...
static PyObject* _draw_pieslice(ImagingDrawObject* self, PyObject* args) {
    double* xy;
    Py_ssize_t n;
    ...
    n = ImagingDrawPieslice(self->image->image, (int)xy[0], (int)xy[1],
                            (int)xy[2], (int)xy[3], start, end, &ink, fill,
                            width, self->blend);
...
static PyObject* _draw_polygon(ImagingDrawObject* self, PyObject* args) {
    double* xy;
    int* ixy;
    Py_ssize_t n, i;
    ...
    int ink;
    int fill = 0;
    if (!PyArg_ParseTuple(args, "Oi|i", &data, &ink, &fill)) return NULL;
    ...
    if (ImagingDrawPolygon(self->image->image, n, ixy, &ink, fill,
                           self->blend) < 0) {
        free(ixy);
        return NULL;
    }
...
static PyObject* _draw_rectangle(ImagingDrawObject* self, PyObject* args) {
    double* xy;
    Py_ssize_t n;
    ...
    n = ImagingDrawRectangle(self->image->image, (int)xy[0], (int)xy[1],
                             (int)xy[2], (int)xy[3], &ink, fill, width,
                             self->blend);
...
static PyObject* pixel_access_new(ImagingObject* imagep, PyObject* args) {
    PixelAccessObject* self;

    int readonly = 0;
    if (!PyArg_ParseTuple(args, "|i", &readonly)) return NULL;
...
static void pixel_access_dealloc(PixelAccessObject* self) {
    Py_XDECREF(self->image);
    PyObject_Del(self);
}

static PyObject* pixel_access_getitem(PixelAccessObject* self, PyObject* xy) {
    int x, y;
    if (_getxy(xy, &x, &y)) return NULL;
...
static int pixel_access_setitem(PixelAccessObject* self, PyObject* xy,
                                PyObject* color) {
    ...
    if (!getink(color, im, ink)) return -1;
...
static PyObject* _effect_mandelbrot(ImagingObject* self, PyObject* args) {
    int xsize = 512;
    int ysize = 512;
    double extent[4];
    int quality = 100;
    ...
    if (!PyArg_ParseTuple(args, "|(ii)(dddd)i", &xsize, &ysize, &extent[0],
                          &extent[1], &extent[2], &extent[3], &quality))
        return NULL;

    return PyImagingNew(ImagingEffectMandelbrot(xsize, ysize, extent, quality));
}

static PyObject* _effect_noise(ImagingObject* self, PyObject* args) {
    int xsize, ysize;
    float sigma = 128;
    if (!PyArg_ParseTuple(args, "(ii)|f", &xsize, &ysize, &sigma)) return NULL;

    return PyImagingNew(ImagingEffectNoise(xsize, ysize, sigma));
}

static PyObject* _effect_spread(ImagingObject* self, PyObject* args) {
    int dist;

    if (!PyArg_ParseTuple(args, "i", &dist)) return NULL;
...
static PyObject* _getcodecstatus(PyObject* self, PyObject* args) {
    int status;
    char* msg;
...
static PyObject* _save_ppm(ImagingObject* self, PyObject* args) {
    char* filename;

    if (!PyArg_ParseTuple(args, "s", &filename)) return NULL;

    if (!ImagingSavePPM(self->image, filename)) return NULL;
...
static PyObject* _getattr_mode(ImagingObject* self, void* closure) {
    return PyUnicode_FromString(self->image->mode);
}

static PyObject* _getattr_size(ImagingObject* self, void* closure) {
    return Py_BuildValue("ii", self->image->xsize, self->image->ysize);
}

static PyObject* _getattr_bands(ImagingObject* self, void* closure) {
    return PyLong_FromLong(self->image->bands);
}

static PyObject* _getattr_id(ImagingObject* self, void* closure) {
    return PyLong_FromSsize_t((Py_ssize_t)self->image);
}

static PyObject* _getattr_ptr(ImagingObject* self, void* closure) {
    return PyCapsule_New(self->image, IMAGING_MAGIC, NULL);
}

static PyObject* _getattr_unsafe_ptrs(ImagingObject* self, void* closure) {
    return Py_BuildValue("(sn)(sn)(sn)", "image8", self->image->image8,
                         "image32", self->image->image32, "image",
                         self->image->image);
};
...
static Py_ssize_t image_length(ImagingObject* self) {
    Imaging im = self->image;

    return (Py_ssize_t)im->xsize * im->ysize;
}

static PyObject* image_item(ImagingObject* self, Py_ssize_t i) {
    int x, y;
    Imaging im = self->image;
...
static PyTypeObject PixelAccess_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "PixelAccess", sizeof(PixelAccessObject), 0,
    /* methods */
...
static PyObject* _get_stats(PyObject* self, PyObject* args) {
    PyObject* d;
    ImagingMemoryArena arena = &ImagingDefaultArena;

    if (!PyArg_ParseTuple(args, ":get_stats")) return NULL;
...
static PyObject* _reset_stats(PyObject* self, PyObject* args) {
    ImagingMemoryArena arena = &ImagingDefaultArena;

    if (!PyArg_ParseTuple(args, ":reset_stats")) return NULL;
...
static PyObject* _get_alignment(PyObject* self, PyObject* args) {
    if (!PyArg_ParseTuple(args, ":get_alignment")) return NULL;

    return PyLong_FromLong(ImagingDefaultArena.alignment);
}

static PyObject* _get_block_size(PyObject* self, PyObject* args) {
    if (!PyArg_ParseTuple(args, ":get_block_size")) return NULL;

    return PyLong_FromLong(ImagingDefaultArena.block_size);
}

static PyObject* _get_blocks_max(PyObject* self, PyObject* args) {
    if (!PyArg_ParseTuple(args, ":get_blocks_max")) return NULL;

    return PyLong_FromLong(ImagingDefaultArena.blocks_max);
}

static PyObject* _set_alignment(PyObject* self, PyObject* args) {
    int alignment;
    if (!PyArg_ParseTuple(args, "i:set_alignment", &alignment)) return NULL;
...
static PyObject* _set_block_size(PyObject* self, PyObject* args) {
    int block_size;
    if (!PyArg_ParseTuple(args, "i:set_block_size", &block_size)) return NULL;
...
static PyObject* _set_blocks_max(PyObject* self, PyObject* args) {
    int blocks_max;
    if (!PyArg_ParseTuple(args, "i:set_blocks_max", &blocks_max)) return NULL;
...
static PyObject* _clear_cache(PyObject* self, PyObject* args) {
    int i = 0;

    if (!PyArg_ParseTuple(args, "|i:clear_cache", &i)) return NULL;
...
static int setup_module(PyObject* m) {
    ...
    /* Ready object types */
    if (PyType_Ready(&Imaging_Type) < 0) return -1;

#ifdef WITH_IMAGEDRAW
    if (PyType_Ready(&ImagingFont_Type) < 0) return -1;

    if (PyType_Ready(&ImagingDraw_Type) < 0) return -1;
#endif
    if (PyType_Ready(&PixelAccess_Type) < 0) return -1;
...
#ifdef HAVE_LIBJPEG
    {
        ...
        PyDict_SetItemString(d, "jpeglib_version",
...
#ifdef HAVE_OPENJPEG
    {
        ...
        PyDict_SetItemString(d, "jp2klib_version",
                             PyUnicode_FromString(ImagingJpeg2KVersion()));
    }
#endif
    ...
    PyModule_AddIntConstant(m, "FIXED", Z_FIXED);
    {
        ...
        PyDict_SetItemString(d, "zlib_version",
...
#ifdef HAVE_LIBTIFF
    {
        ...
        PyDict_SetItemString(d, "libtiff_version",
                             PyUnicode_FromString(ImagingTiffVersion()));
...
#endif
        PyDict_SetItemString(d, "libtiff_support_custom_tags",
                             support_custom_tags);
    }
#endif
...
PyMODINIT_FUNC PyInit__imaging(void) {
    PyObject* m;
    ...
    if (setup_module(m) < 0) return NULL;

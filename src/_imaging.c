            ...
            if (im->bands == 1) {
                ...
                if (rIsInt) {
                    ...
                    if (im->bands == 2) {
                        ...
                        if (!PyArg_ParseTuple(color, "Lii|i", &r, &g, &b,
                                              &a)) {
                            return NULL;
                        }
                    }
...
_convert_matrix(ImagingObject *self, PyObject *args)
{
    char *mode;
    float m[12];
    if (!PyArg_ParseTuple(args, "s(ffff)", &mode, m + 0, m + 1, m + 2,
                          m + 3)) {
        PyErr_Clear();
...
_transform2(ImagingObject *self, PyObject *args)
{
    ...
    imOut = ImagingTransform(self->image, imagep->image, method, x0, y0, x1,
                             y1, a, filter, fill);
...
_draw_arc(ImagingDrawObject *self, PyObject *args)
{
    ...
    float start, end;
    int op = 0;
    if (!PyArg_ParseTuple(args, "Offi|ii", &data, &start, &end, &ink,
                          &width)) {
        return NULL;
    }
...
_draw_chord(ImagingDrawObject *self, PyObject *args)
{
    ...
    n = ImagingDrawChord(self->image->image, (int)xy[0], (int)xy[1],
                         (int)xy[2], (int)xy[3], start, end, &ink, fill, width,
                         self->blend);
...
_effect_mandelbrot(ImagingObject *self, PyObject *args)
{
    ...
    return PyImagingNew(
        ImagingEffectMandelbrot(xsize, ysize, extent, quality));
}
...
#define tostr(a) tostr1(a)
    PyDict_SetItemString(d, "libjpeg_turbo_version",
                         PyUnicode_FromString(tostr(LIBJPEG_TURBO_VERSION)));
#undef tostr
#undef tostr1
#else
    ...
    PyModule_AddObject(m, "HAVE_LIBIMAGEQUANT", Py_True);
    {
        ...
        PyDict_SetItemString(d, "imagequant_version",
                             PyUnicode_FromString(ImagingImageQuantVersion()));
    }
#else

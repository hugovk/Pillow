...
HandleMuxError(WebPMuxError err, char* chunk)
{
    ...
    if (chunk == NULL) {
        message_len = sprintf(
            message, "could not assemble chunks: %s", kErrorMessages[-err]);
    } else {
...
static struct PyMethodDef _anim_encoder_methods[] = {
    ...
    { "assemble",
      (PyCFunction)_anim_encoder_assemble,
      METH_VARARGS,
      "assemble" },
    { NULL, NULL } /* sentinel */
};
...
static struct PyMethodDef _anim_decoder_methods[] = {
    { "get_info",
      (PyCFunction)_anim_decoder_get_info,
      METH_NOARGS,
      "get_info" },
    { "get_chunk",
      (PyCFunction)_anim_decoder_get_chunk,
      METH_VARARGS,
      "get_chunk" },
    { "get_next",
      (PyCFunction)_anim_decoder_get_next,
      METH_NOARGS,
      "get_next" },
...
WebPEncode_wrapper(PyObject* self, PyObject* args)
{
    ...
    if (strcmp(mode, "RGBA") == 0) {
        ...
        {
            ImagingSectionEnter(&cookie);
            ret_size = WebPEncodeRGBA(
                rgb, width, height, 4 * width, quality_factor, &output);
            ImagingSectionLeave(&cookie);
        }
        ...
        {
            ImagingSectionEnter(&cookie);
            ret_size = WebPEncodeRGB(
                rgb, width, height, 3 * width, quality_factor, &output);
            ImagingSectionLeave(&cookie);
        }
    } else {
        Py_RETURN_NONE;

...
_setimage(ImagingEncoderObject* encoder, PyObject* args)
{
    ...
    if (state->xsize <= 0 || state->xsize + state->xoff > im->xsize || state->ysize <= 0 || state->ysize + state->yoff > im->ysize) {
...
PyImaging_LibTiffEncoderNew(PyObject* self, PyObject* args)
{
    ...
    for (pos = 0; pos < tags_size; pos++) {
        ...
        if (PyBytes_Check(value) && (type == TIFF_BYTE || type == TIFF_UNDEFINED)) {

...
static PyObject *apply(PyObject *self, PyObject *args) {
    ...
    for (row_idx = 0; row_idx < height; row_idx++) {
        ...
        if (row_idx == 0 || row_idx == height - 1) {
            for (col_idx = 0; col_idx < width; col_idx++) outrow[col_idx] = 0;
            continue;
        }
        ...
        for (col_idx = 1; col_idx < width - 1; col_idx++) {
            ...
            int lut_idx = (b0 | (b1 << 1) | (b2 << 2) | (b3 << 3) | (b4 << 4) |
                           (b5 << 5) | (b6 << 6) | (b7 << 7) | (b8 << 8));
...
*/
static PyObject *get_on_pixels(PyObject *self, PyObject *args) {
    Py_ssize_t i0;
    Imaging img;
    UINT8 **rows;
    int row_idx, col_idx;
...
PyMODINIT_FUNC PyInit__imagingmorph(void) {
    PyObject *m;

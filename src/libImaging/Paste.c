...
static inline void paste(Imaging imOut, Imaging imIn, int dx, int dy, int sx,
...
static inline void paste_mask_1(Imaging imOut, Imaging imIn, Imaging imMask,
                                int dx, int dy, int sx, int sy, int xsize,
                                int ysize, int pixelsize) {
...
static inline void paste_mask_L(Imaging imOut, Imaging imIn, Imaging imMask,
                                int dx, int dy, int sx, int sy, int xsize,
                                int ysize, int pixelsize) {
...
static inline void paste_mask_RGBA(Imaging imOut, Imaging imIn, Imaging imMask,
                                   int dx, int dy, int sx, int sy, int xsize,
                                   int ysize, int pixelsize) {
...
int ImagingPaste(Imaging imOut, Imaging imIn, Imaging imMask, int dx0, int dy0,
                 int dx1, int dy1) {
    int xsize, ysize;
    int pixelsize;
    int sx0, sy0;
    ...
    sx0 = sy0 = 0;
    if (dx0 < 0) xsize += dx0, sx0 = -dx0, dx0 = 0;
    if (dx0 + xsize > imOut->xsize) xsize = imOut->xsize - dx0;
    if (dy0 < 0) ysize += dy0, sy0 = -dy0, dy0 = 0;
    if (dy0 + ysize > imOut->ysize) ysize = imOut->ysize - dy0;
    ...
    if (!imMask) {
        ...
        ImagingSectionEnter(&cookie);
        paste_mask_1(imOut, imIn, imMask, dx0, dy0, sx0, sy0, xsize, ysize,
                     pixelsize);
        ImagingSectionLeave(&cookie);

    } else if (strcmp(imMask->mode, "L") == 0) {
        ImagingSectionEnter(&cookie);
        paste_mask_L(imOut, imIn, imMask, dx0, dy0, sx0, sy0, xsize, ysize,
                     pixelsize);
        ImagingSectionLeave(&cookie);
...
static inline void fill(Imaging imOut, const void* ink_, int dx, int dy,
    ...
    if (imOut->image8 || ink32 == 0L) {
        ...
        for (y = 0; y < ysize; y++) {
            INT32* out = imOut->image32[y + dy] + dx;
            for (x = 0; x < xsize; x++) out[x] = ink32;
        }
    }
}

static inline void fill_mask_1(Imaging imOut, const void* ink_, Imaging imMask,
                               int dx, int dy, int sx, int sy, int xsize,
                               int ysize, int pixelsize) {
...
static inline void fill_mask_L(Imaging imOut, const UINT8* ink, Imaging imMask,
                               int dx, int dy, int sx, int sy, int xsize,
                               int ysize, int pixelsize) {
...
static inline void fill_mask_RGBA(Imaging imOut, const UINT8* ink,
                                  Imaging imMask, int dx, int dy, int sx,
                                  int sy, int xsize, int ysize, int pixelsize) {
...
int ImagingFill2(Imaging imOut, const void* ink, Imaging imMask, int dx0,
                 int dy0, int dx1, int dy1) {
    ImagingSectionCookie cookie;
    int xsize, ysize;
    int pixelsize;
    int sx0, sy0;
    ...
    if (!imMask) {
        ...
        ImagingSectionEnter(&cookie);
        fill_mask_1(imOut, ink, imMask, dx0, dy0, sx0, sy0, xsize, ysize,
                    pixelsize);
        ImagingSectionLeave(&cookie);

    } else if (strcmp(imMask->mode, "L") == 0) {
        ImagingSectionEnter(&cookie);
        fill_mask_L(imOut, ink, imMask, dx0, dy0, sx0, sy0, xsize, ysize,
                    pixelsize);
        ImagingSectionLeave(&cookie);

    } else if (strcmp(imMask->mode, "RGBA") == 0) {
        ImagingSectionEnter(&cookie);
        fill_mask_RGBA(imOut, ink, imMask, dx0, dy0, sx0, sy0, xsize, ysize,
                       pixelsize);
        ImagingSectionLeave(&cookie);

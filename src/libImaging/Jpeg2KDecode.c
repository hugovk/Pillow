...
static OPJ_SIZE_T j2k_read(void *p_buffer, OPJ_SIZE_T p_nb_bytes,
                           void *p_user_data) {
...
typedef void (*j2k_unpacker_t)(opj_image_t *in, const JPEG2KTILEINFO *tileInfo,
...
static void j2ku_graya_la(opj_image_t *in, const JPEG2KTILEINFO *tileinfo,
    ...
    if (shift < 0) offset += 1 << (-shift - 1);
    if (ashift < 0) aoffset += 1 << (-ashift - 1);
...
static void j2ku_srgb_rgb(opj_image_t *in, const JPEG2KTILEINFO *tileinfo,
    ...
    for (n = 0; n < 3; ++n) {
        ...
        if (csiz[n] == 3) csiz[n] = 4;

        if (shifts[n] < 0) offsets[n] += 1 << (-shifts[n] - 1);
    ...
    for (y = 0; y < h; ++y) {
        const UINT8 *data[3];
        UINT8 *row = (UINT8 *)im->image[y0 + y] + x0 * 4;
        for (n = 0; n < 3; ++n) data[n] = &cdata[n][csiz[n] * y * w];
...
static void j2ku_srgba_rgba(opj_image_t *in, const JPEG2KTILEINFO *tileinfo,
    ...
    for (y = 0; y < h; ++y) {
        const UINT8 *data[4];
        UINT8 *row = (UINT8 *)im->image[y0 + y] + x0 * 4;
        for (n = 0; n < 4; ++n) data[n] = &cdata[n][csiz[n] * y * w];
    ...
    */
    ...
    for (;;) {
        ...
        if (tile_info.x0 >= tile_info.x1 || tile_info.y0 >= tile_info.y1 ||
            tile_info.x0 < image->x0 || tile_info.y0 < image->y0 ||
...
int ImagingJpeg2KDecode(Imaging im, ImagingCodecState state, UINT8 *buf,
                        Py_ssize_t bytes) {
    if (bytes) {
        state->errcode = IMAGING_CODEC_BROKEN;
        state->state = J2K_STATE_FAILED;
...
const char *ImagingJpeg2KVersion(void) { return opj_version(); }

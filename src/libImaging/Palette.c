...
ImagingPalette ImagingPaletteNew(const char* mode) {
    ...
    if (!palette) return (ImagingPalette)ImagingError_MemoryError();
...
ImagingPalette ImagingPaletteNewBrowser(void) {
...
ImagingPalette ImagingPaletteDuplicate(ImagingPalette palette) {
    ...
    if (!new_palette) return (ImagingPalette)ImagingError_MemoryError();
...
void ImagingPaletteDelete(ImagingPalette palette) {
    /* Destroy palette object */

    if (palette) {
        if (palette->cache) free(palette->cache);
        free(palette);
    }
}
...
void ImagingPaletteCacheUpdate(ImagingPalette palette, int r, int g, int b) {
    int i, j;
    ...
    for (i = 0; i < 256; i++) {
        ...
        dmin[i] = tmin;
        if (tmax < dmax) dmax = tmax; /* keep the smallest max distance only */
    }
    ...
    for (i = 0; i < BOXVOLUME; i++) d[i] = (unsigned int)~0;
...
int ImagingPaletteCachePrepare(ImagingPalette palette) {
    ...
    if (palette->cache == NULL) {
        ...
        for (i = 0; i < entries; i++) palette->cache[i] = 0x100;
    }

    return 0;
}

void ImagingPaletteCacheDelete(ImagingPalette palette) {

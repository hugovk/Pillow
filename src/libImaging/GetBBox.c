...
int ImagingGetBBox(Imaging im, int bbox[4]) {
...
#define GETBBOX(image, mask)                       \
    for (y = 0; y < im->ysize; y++) {              \
        ...
        for (x = 0; x < im->xsize; x++)            \
            if (im->image[y][x] & mask) {          \
                ...
                if (x >= bbox[2]) bbox[2] = x + 1; \
            }                                      \
        if (has_data) {                            \
            if (bbox[1] < 0) bbox[1] = y;          \
            bbox[3] = y + 1;                       \
        }                                          \
    }
    ...
    if (bbox[1] < 0) return 0; /* no data */

    return 1; /* ok */
}

int ImagingGetProjection(Imaging im, UINT8* xproj, UINT8* yproj) {
...
int ImagingGetExtrema(Imaging im, void* extrema) {
    int x, y;
    INT32 imin, imax;
    FLOAT32 fmin, fmax;
    ...
    if (!im->xsize || !im->ysize) return 0; /* zero size */
...
ImagingColorItem* ImagingGetColors(Imaging im, int maxcolors, int* size) {
...
static ImagingColorItem* getcolors32(Imaging im, int maxcolors, int* size) {
    ...
    if (!code_size) return ImagingError_MemoryError(); /* just give up */
    ...
    if (im->bands == 3) ((UINT8*)&pixel_mask)[3] = 0;
    ...
    for (y = 0; y < im->ysize; y++) {
        ...
        for (x = 0; x < im->xsize; x++) {
            ...
            if (!v->count) {
                /* add to table */
                if (colors++ == maxcolors) goto overflow;
                v->x = x;
                v->y = y;
                v->pixel = pixel;
                v->count = 1;
            ...
            for (;;) {
                ...
                incr = incr << 1;
                if (incr > code_mask) incr = incr ^ code_poly;
            }
        }
    }

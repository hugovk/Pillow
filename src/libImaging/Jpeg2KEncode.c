...
static void j2k_warn(const char *msg, void *client_data) {
    // Null handler
}
...
static OPJ_SIZE_T j2k_write(void *p_buffer, OPJ_SIZE_T p_nb_bytes,
                            void *p_user_data) {
...
static OPJ_BOOL j2k_seek(OPJ_OFF_T p_nb_bytes, void *p_user_data) {
    ImagingCodecState state = (ImagingCodecState)p_user_data;
    off_t pos = 0;
...
static void j2k_pack_l(Imaging im, UINT8 *buf, unsigned x0, unsigned y0,
                       unsigned w, unsigned h) {
    UINT8 *ptr = buf;
    unsigned x, y;
    for (y = 0; y < h; ++y) {
        UINT8 *data = (UINT8 *)(im->image[y + y0] + x0);
        for (x = 0; x < w; ++x) *ptr++ = *data++;
    }
}

static void j2k_pack_i16(Imaging im, UINT8 *buf, unsigned x0, unsigned y0,
                         unsigned w, unsigned h) {
    UINT8 *ptr = buf;
    unsigned x, y;
    for (y = 0; y < h; ++y) {
...
static void j2k_pack_la(Imaging im, UINT8 *buf, unsigned x0, unsigned y0,
                        unsigned w, unsigned h) {
    UINT8 *ptr = buf;
    UINT8 *ptra = buf + w * h;
    unsigned x, y;
...
static void j2k_pack_rgb(Imaging im, UINT8 *buf, unsigned x0, unsigned y0,
                         unsigned w, unsigned h) {
    UINT8 *pr = buf;
    UINT8 *pg = pr + w * h;
    UINT8 *pb = pg + w * h;
...
static void j2k_pack_rgba(Imaging im, UINT8 *buf, unsigned x0, unsigned y0,
                          unsigned w, unsigned h) {
    UINT8 *pr = buf;
    UINT8 *pg = pr + w * h;
    UINT8 *pb = pg + w * h;
...
static void j2k_set_cinema_params(Imaging im, int components,
    ...
    if (params->cp_cinema == OPJ_CINEMA4K_24) {
        ...
        for (n = 0; n < params->tcp_numlayers; ++n) {
            ...
            if (params->tcp_rates[0] == 0) {
                ...
                if (rate > CINEMA_24_CS_LENGTH) params->tcp_rates[n] = max_rate;
            }
        }
        ...
        for (n = 0; n < params->tcp_numlayers; ++n) {
            ...
            if (params->tcp_rates[0] == 0) {
                ...
                rate = ((float)(components * im->xsize * im->ysize * 8) /
                        (params->tcp_rates[n] * 8));
                if (rate > CINEMA_48_CS_LENGTH) params->tcp_rates[n] = max_rate;
            }
        }
...
static int j2k_encode_entry(Imaging im, ImagingCodecState state) {
    ...
    switch (params.cp_cinema) {
        ...
        case OPJ_CINEMA2K_48:
            ...
            if (params.numresolution > 6) params.numresolution = 6;
            break;
        case OPJ_CINEMA4K_24:
            params.cp_rsiz = OPJ_CINEMA4K;
            if (params.numresolution > 7) params.numresolution = 7;
            break;
    }
    ...
    for (y = 0; y < tiles_y; ++y) {
        ...
        if (ty0 < params.image_offset_y0) ty0 = params.image_offset_y0;
        if (ty1 > ysiz) ty1 = ysiz;
        ...
        for (x = 0; x < tiles_x; ++x) {
            ...
            if (tx0 < params.image_offset_x0) tx0 = params.image_offset_x0;
            if (tx1 > xsiz) tx1 = xsiz;
...
int ImagingJpeg2KEncode(Imaging im, ImagingCodecState state, UINT8 *buf,
                        int bytes) {
    if (state->state == J2K_STATE_FAILED) return -1;
...
int ImagingJpeg2KEncodeCleanup(ImagingCodecState state) {
    JPEG2KENCODESTATE *context = (JPEG2KENCODESTATE *)state->context;
    ...
    if (context->error_msg) free((void *)context->error_msg);

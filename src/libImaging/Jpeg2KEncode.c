...
j2k_encode_entry(Imaging im, ImagingCodecState state)
{
    ...
    for (y = 0; y < tiles_y; ++y) {
        ...
        for (x = 0; x < tiles_x; ++x) {
            ...
            if (!opj_write_tile(
                    codec, tile_ndx++, state->buffer, data_size, stream)) {
                state->errcode = IMAGING_CODEC_BROKEN;
                state->state = J2K_STATE_FAILED;
                goto quick_exit;
            }

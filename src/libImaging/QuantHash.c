...
static int _hashtable_insert_node(HashTable *, HashNode *, int, int,
                                  CollisionFunc);
...
static void _hashtable_rehash(HashTable *h, CollisionFunc cf,
                              uint32_t newSize) {
...
static int _hashtable_insert_node(HashTable *h, HashNode *node, int resize,
                                  int update, CollisionFunc cf) {
...
static int _hashtable_insert(HashTable *h, HashKey_t key, HashVal_t val,
                             int resize, int update) {
    HashNode **n, *nv;
    HashNode *t;
    int i;
...
int hashtable_insert_or_update_computed(HashTable *h, HashKey_t key,
                                        ComputeFunc newFunc,
...
uint32_t hashtable_get_count(const HashTable *h) { return h->count; }

void *hashtable_get_user_data(const HashTable *h) { return h->userData; }

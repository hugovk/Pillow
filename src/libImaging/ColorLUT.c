    ...
    INT16* table)
{
    ...
    if (imIn->type != IMAGING_TYPE_UINT8 || imOut->type != IMAGING_TYPE_UINT8 || imIn->bands < 3 || imOut->bands < table_channels) {
    ...
    for (y = 0; y < imOut->ysize; y++) {
        ...
        for (x = 0; x < imOut->xsize; x++) {
            ...
            int idx = table_channels * table_index3D(index1D >> SCALE_BITS, index2D >> SCALE_BITS, index3D >> SCALE_BITS, size1D, size1D_2D);

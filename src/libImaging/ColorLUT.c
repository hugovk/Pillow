...
static inline void interpolate3(INT16 out[3], const INT16 a[3],
                                const INT16 b[3], INT16 shift) {
...
static inline void interpolate4(INT16 out[4], const INT16 a[4],
                                const INT16 b[4], INT16 shift) {
    out[0] = (a[0] * ((1 << SHIFT_BITS) - shift) + b[0] * shift) >> SHIFT_BITS;
    out[1] = (a[1] * ((1 << SHIFT_BITS) - shift) + b[1] * shift) >> SHIFT_BITS;
    out[2] = (a[2] * ((1 << SHIFT_BITS) - shift) + b[2] * shift) >> SHIFT_BITS;
...
static inline int table_index3D(int index1D, int index2D, int index3D,
...
*/
Imaging ImagingColorLUT3D_linear(Imaging imOut, Imaging imIn,
                                 int table_channels, int size1D, int size2D,
                                 int size3D, INT16* table) {

...
char *ImagingGetModeDIB(int size_out[2]) {
    ...
    if (!(GetDeviceCaps(dc, RASTERCAPS) & RC_PALETTE)) {
        mode = "RGB";
        if (GetDeviceCaps(dc, BITSPIXEL) == 1) mode = "1";
    }
...
ImagingDIB ImagingNewDIB(const char *mode, int xsize, int ysize) {
    ...
    if (!dib) return (ImagingDIB)ImagingError_MemoryError();
    /* malloc check ok, small constant allocation */
    dib->info =
...
void ImagingExposeDIB(ImagingDIB dib, void *dc) {
    ...
    if (dib->palette != 0) SelectPalette((HDC)dc, dib->palette, FALSE);
...
void ImagingDrawDIB(ImagingDIB dib, void *dc, int dst[4], int src[4]) {
    ...
    if (GetDeviceCaps((HDC)dc, RASTERCAPS) & RC_STRETCHDIB) {
        ...
        StretchDIBits((HDC)dc, dst[0], dst[1], dst[2] - dst[0], dst[3] - dst[1],
                      src[0], src[1], src[2] - src[0], src[3] - src[1],
                      dib->bits, dib->info, DIB_RGB_COLORS, SRCCOPY);
    } else {
        /* stretchblt (displays) */
        if (dib->palette != 0) SelectPalette((HDC)dc, dib->palette, FALSE);
...
void ImagingDeleteDIB(ImagingDIB dib) {
    ...
    if (dib->palette) DeleteObject(dib->palette);
    if (dib->bitmap) {
        SelectObject(dib->dc, dib->old_bitmap);
        DeleteObject(dib->bitmap);
    }
    if (dib->dc) DeleteDC(dib->dc);
    free(dib->info);
}

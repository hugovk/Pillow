...
Imaging ImagingAllocateArray(Imaging im, int dirty, int block_size) {
    ...
    if (lines_per_block == 0) lines_per_block = 1;
...
static void ImagingDestroyBlock(Imaging im) {
    if (im->block) free(im->block);
}

Imaging ImagingAllocateBlock(Imaging im) {
    Py_ssize_t y, i;
...
Imaging ImagingNewInternal(const char *mode, int xsize, int ysize, int dirty) {
    Imaging im;
...
Imaging ImagingNewDirty(const char *mode, int xsize, int ysize) {
...
Imaging ImagingNew2Dirty(const char *mode, Imaging imOut, Imaging imIn) {
...
void ImagingCopyPalette(Imaging destination, Imaging source) {
    if (source->palette) {
        if (destination->palette) ImagingPaletteDelete(destination->palette);

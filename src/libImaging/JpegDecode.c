...
int ImagingJpegUseJCSExtensions()
{
...
int ImagingJpegDecode(Imaging im, ImagingCodecState state, UINT8* buf, Py_ssize_t bytes)
{
...
#endif
        else if (strcmp(context->rawmode, "CMYK") == 0 || strcmp(context->rawmode, "CMYK;I") == 0)
    ...
    case 3:
        ...
        while (state->y < state->ysize) {
            ...
            if (ok != 1)
                break;
            state->shuffle((UINT8*)im->image[state->y + state->yoff] + state->xoff * im->pixelsize, state->buffer,
                state->xsize);
            state->y++;
        }
        if (ok != 1)

...
static inline UINT8 clip8(float in) {
    if (in <= 0.0) return 0;
    if (in >= 255.0) return 255;
    return (UINT8)in;
}

Imaging ImagingExpand(Imaging imIn, int xmargin, int ymargin, int mode) {
    Imaging imOut;
    int x, y;
    ...
    imOut = ImagingNewDirty(imIn->mode, imIn->xsize + 2 * xmargin,
                            imIn->ysize + 2 * ymargin);
...
#define EXPAND(type, image)                                           \
    {                                                                 \
        for (y = 0; y < ymargin; y++) EXPAND_LINE(type, image, 0, y); \
...
void ImagingFilter3x3(Imaging imOut, Imaging im, const float* kernel,
                      float offset) {
...
void ImagingFilter5x5(Imaging imOut, Imaging im, const float* kernel,
                      float offset) {
...
Imaging ImagingFilter(Imaging im, int xsize, int ysize, const FLOAT32* kernel,
                      FLOAT32 offset) {
    Imaging imOut;
    ImagingSectionCookie cookie;
    ...
    if (im->xsize < xsize || im->ysize < ysize) return ImagingCopy(im);

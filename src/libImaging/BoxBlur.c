...
void static inline ImagingLineBoxBlur8(UINT8 *lineOut, UINT8 *lineIn, int lastx,
                                       int radius, int edgeA, int edgeB,
                                       UINT32 ww, UINT32 fw) {
    int x;
    UINT32 acc;
    UINT32 bulk;
...
Imaging ImagingHorizontalBoxBlur(Imaging imOut, Imaging imIn,
                                 float floatRadius) {
    ...
    if (lineOut == NULL) return ImagingError_MemoryError();
...
Imaging ImagingBoxBlur(Imaging imOut, Imaging imIn, float radius, int n) {
    ...
    if (imIn->type != IMAGING_TYPE_UINT8) return ImagingError_ModeError();

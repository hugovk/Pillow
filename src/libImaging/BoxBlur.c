...
void static inline ImagingLineBoxBlur32(pixel* lineOut,
                                        pixel* lineIn,
                                        int lastx,
                                        int radius,
                                        int edgeA,
...
void static inline ImagingLineBoxBlur8(UINT8* lineOut,
                                       UINT8* lineIn,
                                       int lastx,
                                       int radius,
                                       int edgeA,
...
Imaging ImagingHorizontalBoxBlur(Imaging imOut,
                                 Imaging imIn,
                                 float floatRadius) {
    ...
    if (imIn->image8) {
        for (y = 0; y < imIn->ysize; y++) {
            ImagingLineBoxBlur8(
                ...
                imIn->image8[y], imIn->xsize - 1, radius, edgeA, edgeB, ww, fw);
            if (imIn == imOut) {
                // Commit.
        ...
        for (y = 0; y < imIn->ysize; y++) {
            ImagingLineBoxBlur32(
                ...
                (pixel*)imIn->image32[y], imIn->xsize - 1, radius, edgeA, edgeB,
                ww, fw);
            if (imIn == imOut) {
                // Commit.
...
Imaging ImagingBoxBlur(Imaging imOut, Imaging imIn, float radius, int n) {
    int i;
...
Imaging ImagingGaussianBlur(Imaging imOut,
                            Imaging imIn,
                            float radius,
                            int passes) {
    float sigma2, L, l, a;

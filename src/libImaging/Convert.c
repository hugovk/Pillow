...
static void bit2l(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++) *out++ = (*in++ != 0) ? 255 : 0;
}

static void bit2rgb(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++) {
        UINT8 v = (*in++ != 0) ? 255 : 0;
        *out++ = v;
...
static void bit2cmyk(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++) {
        *out++ = 0;
        *out++ = 0;
...
static void bit2ycbcr(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++) {
        *out++ = (*in++ != 0) ? 255 : 0;
        *out++ = 128;
...
static void bit2hsv(UINT8* out, const UINT8* in, int xsize) {
    int x;
...
static void l2bit(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++) *out++ = (*in++ >= 128) ? 255 : 0;
}

static void lA2la(UINT8* out, const UINT8* in, int xsize) {
    int x;
...
static void l2la(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++) {
        UINT8 v = *in++;
        *out++ = v;
...
static void l2rgb(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++) {
        UINT8 v = *in++;
        *out++ = v;
...
static void l2hsv(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, out += 4) {
        UINT8 v = *in++;
        out[0] = 0;
...
static void la2l(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 4) *out++ = in[0];
}

static void la2rgb(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 4) {
        UINT8 v = in[0];
        *out++ = v;
...
static void la2hsv(UINT8* out, const UINT8* in, int xsize) {
    int x;
...
static void rgb2bit(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 4)
        /* ITU-R Recommendation 601-2 (assuming nonlinear RGB) */
...
static void rgb2l(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 4)
        /* ITU-R Recommendation 601-2 (assuming nonlinear RGB) */
        *out++ = L24(in) >> 16;
...
static void rgb2la(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 4, out += 4) {
        /* ITU-R Recommendation 601-2 (assuming nonlinear RGB) */
...
static void rgb2i(UINT8* out_, const UINT8* in, int xsize) {
    int x;
...
static void rgb2f(UINT8* out_, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 4, out_ += 4) {
...
static void rgb2bgr15(UINT8* out_, const UINT8* in, int xsize) {
    int x;
...
static void rgb2bgr16(UINT8* out_, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 4, out_ += 2) {
...
static void rgb2bgr24(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 4) {
        *out++ = in[2];
        *out++ = in[1];
...
static void rgb2hsv_row(UINT8* out, const UINT8* in) {  // following colorsys.py
...
static void rgb2hsv(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 4, out += 4) {
        rgb2hsv_row(out, in);
        out[3] = in[3];
...
static void hsv2rgb(UINT8* out, const UINT8* in,
                    int xsize) {  // following colorsys.py
...
static void rgb2rgba(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++) {
        *out++ = *in++;
        *out++ = *in++;
...
static void rgba2la(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 4, out += 4) {
        /* ITU-R Recommendation 601-2 (assuming nonlinear RGB) */
        out[0] = out[1] = out[2] = L24(in) >> 16;
...
static void rgba2rgb(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++) {
        *out++ = *in++;
        *out++ = *in++;
...
static void rgbA2rgba(UINT8* out, const UINT8* in, int xsize) {
    int x;
...
static void rgbT2rgba(UINT8* out, int xsize, int r, int g, int b) {
...
static void l2cmyk(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++) {
        *out++ = 0;
        *out++ = 0;
...
static void la2cmyk(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 4) {
        *out++ = 0;
        *out++ = 0;
...
static void rgb2cmyk(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++) {
...
static void cmyk2rgb(UINT8* out, const UINT8* in, int xsize) {
    int x, nk, tmp;
    for (x = 0; x < xsize; x++) {
        nk = 255 - in[3];
...
static void cmyk2hsv(UINT8* out, const UINT8* in, int xsize) {
    int x, nk, tmp;
    for (x = 0; x < xsize; x++) {
        nk = 255 - in[3];
        out[0] = CLIP8(nk - MULDIV255(in[0], nk, tmp));
...
static void bit2i(UINT8* out_, const UINT8* in, int xsize) {
    int x;
...
static void l2i(UINT8* out_, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, out_ += 4) {
        INT32 v = *in++;
        memcpy(out_, &v, sizeof(v));
...
static void i2l(UINT8* out, const UINT8* in_, int xsize) {
    int x;
...
static void i2f(UINT8* out_, const UINT8* in_, int xsize) {
    int x;
...
static void i2rgb(UINT8* out, const UINT8* in_, int xsize) {
    int x;
    INT32* in = (INT32*)in_;
...
static void i2hsv(UINT8* out, const UINT8* in_, int xsize) {
    int x;
    INT32* in = (INT32*)in_;
    for (x = 0; x < xsize; x++, in++, out += 4) {
        out[0] = 0;
...
static void bit2f(UINT8* out_, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, out_ += 4) {
...
static void l2f(UINT8* out_, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, out_ += 4) {
...
static void f2l(UINT8* out, const UINT8* in_, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, out++, in_ += 4) {
        FLOAT32 v;
        memcpy(&v, in_, sizeof(v));
...
static void f2i(UINT8* out_, const UINT8* in_, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in_ += 4, out_ += 4) {
        FLOAT32 f;
        INT32 i;
...
static void l2ycbcr(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++) {
        *out++ = *in++;
        *out++ = 128;
...
static void la2ycbcr(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 4) {
        *out++ = in[0];
        *out++ = 128;
...
static void ycbcr2l(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 4) *out++ = in[0];
}

static void ycbcr2la(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 4, out += 4) {
...
static void I_I16L(UINT8* out, const UINT8* in_, int xsize) {
    int x, v;
...
static void I_I16B(UINT8* out, const UINT8* in_, int xsize) {
    int x, v;
    for (x = 0; x < xsize; x++, in_ += 4) {
        INT32 i;
        memcpy(&i, in_, sizeof(i));
...
static void I16L_I(UINT8* out_, const UINT8* in, int xsize) {
    int x;
...
static void I16B_I(UINT8* out_, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 2, out_ += 4) {
...
static void I16L_F(UINT8* out_, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 2, out_ += 4) {
...
static void I16B_F(UINT8* out_, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 2, out_ += 4) {
...
static void L_I16L(UINT8* out, const UINT8* in, int xsize) {
    int x;
...
static void L_I16B(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in++) {
        *out++ = 0;
        *out++ = *in;
...
static void I16L_L(UINT8* out, const UINT8* in, int xsize) {
    int x;
...
static void I16B_L(UINT8* out, const UINT8* in, int xsize) {
    int x;
    for (x = 0; x < xsize; x++, in += 2)
        if (in[0] != 0)
            *out++ = 255;
...
static void p2bit(UINT8* out, const UINT8* in, int xsize,
                  const UINT8* palette) {
    int x;
...
static void pa2bit(UINT8* out, const UINT8* in, int xsize,
                   const UINT8* palette) {
    int x;
    /* FIXME: precalculate greyscale palette? */
    for (x = 0; x < xsize; x++, in += 4)
...
static void p2l(UINT8* out, const UINT8* in, int xsize, const UINT8* palette) {
    int x;
    /* FIXME: precalculate greyscale palette? */
    for (x = 0; x < xsize; x++) *out++ = L(&palette[in[x] * 4]) / 1000;
}

static void pa2l(UINT8* out, const UINT8* in, int xsize, const UINT8* palette) {
    int x;
    /* FIXME: precalculate greyscale palette? */
    for (x = 0; x < xsize; x++, in += 4) *out++ = L(&palette[in[0] * 4]) / 1000;
}

static void p2pa(UINT8* out, const UINT8* in, int xsize, const UINT8* palette) {
    int x;
    for (x = 0; x < xsize; x++, in++) {
...
static void p2la(UINT8* out, const UINT8* in, int xsize, const UINT8* palette) {
    int x;
    /* FIXME: precalculate greyscale palette? */
    for (x = 0; x < xsize; x++, out += 4) {
...
static void pa2la(UINT8* out, const UINT8* in, int xsize,
                  const UINT8* palette) {
    int x;
    /* FIXME: precalculate greyscale palette? */
    for (x = 0; x < xsize; x++, in += 4, out += 4) {
...
static void p2i(UINT8* out_, const UINT8* in, int xsize, const UINT8* palette) {
    int x;
    for (x = 0; x < xsize; x++, out_ += 4) {
...
static void pa2i(UINT8* out_, const UINT8* in, int xsize,
                 const UINT8* palette) {
    int x;
...
static void p2f(UINT8* out_, const UINT8* in, int xsize, const UINT8* palette) {
    int x;
    for (x = 0; x < xsize; x++, out_ += 4) {
...
static void pa2f(UINT8* out_, const UINT8* in, int xsize,
                 const UINT8* palette) {
    int x;
...
static void p2rgb(UINT8* out, const UINT8* in, int xsize,
                  const UINT8* palette) {
    int x;
    for (x = 0; x < xsize; x++) {
...
static void pa2rgb(UINT8* out, const UINT8* in, int xsize,
                   const UINT8* palette) {
    int x;
    for (x = 0; x < xsize; x++, in += 4) {
...
static void p2hsv(UINT8* out, const UINT8* in, int xsize,
                  const UINT8* palette) {
    int x;
    for (x = 0; x < xsize; x++, out += 4) {
        const UINT8* rgb = &palette[*in++ * 4];
...
static void pa2hsv(UINT8* out, const UINT8* in, int xsize,
                   const UINT8* palette) {
    int x;
    for (x = 0; x < xsize; x++, in += 4, out += 4) {
        const UINT8* rgb = &palette[in[0] * 4];
...
static void p2rgba(UINT8* out, const UINT8* in, int xsize,
                   const UINT8* palette) {
    int x;
    for (x = 0; x < xsize; x++) {
        const UINT8* rgba = &palette[*in++ * 4];
...
static void pa2rgba(UINT8* out, const UINT8* in, int xsize,
                    const UINT8* palette) {
    int x;
    for (x = 0; x < xsize; x++, in += 4) {
        const UINT8* rgb = &palette[in[0] * 4];
...
static void p2cmyk(UINT8* out, const UINT8* in, int xsize,
                   const UINT8* palette) {
    p2rgb(out, in, xsize, palette);
    rgb2cmyk(out, out, xsize);
}

static void pa2cmyk(UINT8* out, const UINT8* in, int xsize,
                    const UINT8* palette) {
    pa2rgb(out, in, xsize, palette);
    rgb2cmyk(out, out, xsize);
}

static void p2ycbcr(UINT8* out, const UINT8* in, int xsize,
                    const UINT8* palette) {
    p2rgb(out, in, xsize, palette);
    ImagingConvertRGB2YCbCr(out, out, xsize);
}

static void pa2ycbcr(UINT8* out, const UINT8* in, int xsize,
                     const UINT8* palette) {
    pa2rgb(out, in, xsize, palette);
    ImagingConvertRGB2YCbCr(out, out, xsize);
}

static Imaging frompalette(Imaging imOut, Imaging imIn, const char* mode) {
    ...
    if (!imIn->palette) return (Imaging)ImagingError_ValueError("no palette");
...
#pragma optimize("", off)
#endif
static Imaging topalette(Imaging imOut, Imaging imIn, const char* mode,
    ...
    if (!palette) return (Imaging)ImagingError_ValueError("no palette");

    imOut = ImagingNew2Dirty(mode, imOut, imIn);
    if (!imOut) {
        if (palette != inpalette) ImagingPaletteDelete(palette);
        return NULL;
    }
    ...
    if (imIn->bands == 1) {
        ...
        if (inpalette != palette) ImagingPaletteCacheDelete(palette);
    }

    if (inpalette != palette) ImagingPaletteDelete(palette);
...
static Imaging convert(Imaging imOut, Imaging imIn, const char* mode,
    ...
    if (!mode) {
        ...
        if (!imIn->palette) return (Imaging)ImagingError_ModeError();
    ...
    if (dither && strcmp(mode, "1") == 0) return tobilevel(imOut, imIn, dither);
...
Imaging ImagingConvert(Imaging imIn, const char* mode, ImagingPalette palette,
                       int dither) {
    return convert(NULL, imIn, mode, palette, dither);
}

Imaging ImagingConvert2(Imaging imOut, Imaging imIn) {
    return convert(imOut, imIn, imOut->mode, NULL, 0);
}

Imaging ImagingConvertTransparent(Imaging imIn, const char* mode, int r, int g,
                                  int b) {
    ImagingSectionCookie cookie;
    ImagingShuffler convert;
    Imaging imOut = NULL;
...
Imaging ImagingConvertInPlace(Imaging imIn, const char* mode) {
    ImagingSectionCookie cookie;
    ImagingShuffler convert;
    int y;

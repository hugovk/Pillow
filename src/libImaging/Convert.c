        ...
        int dither)
{
...
#else
    {
        ...
        sprintf(
            buf, "conversion from %s to %s not supported", imIn->mode, mode);
    ...
    for (y = 0; y < imIn->ysize; y++)
        (*convert)(
            (UINT8*)imOut->image[y], (UINT8*)imIn->image[y], imIn->xsize);
    ImagingSectionLeave(&cookie);

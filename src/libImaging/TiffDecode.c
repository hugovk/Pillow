...
int
ReadTile(TIFF* tiff, UINT32 col, UINT32 row, UINT32* buffer)
{
...
int
ReadStrip(TIFF* tiff, UINT32 row, UINT32* buffer)
{
    ...
    if (TIFFReadEncodedStrip(
            tiff, TIFFComputeStrip(tiff, row, 0), (tdata_t)buffer, -1) == -1) {
...
int
ImagingLibTiffDecode(Imaging im,
                     ...
                     Py_ssize_t bytes)
{
    ...
    if (TIFFIsTiled(tiff)) {
        ...
        if (TIFFTileSize(tiff) > state->bytes) {
            // If the strip size as expected by LibTiff isn't what we're
            // expecting, abort.
            state->errcode = IMAGING_CODEC_MEMORY;
            TIFFClose(tiff);
            return -1;
        ...
        for (y = state->yoff; y < state->ysize; y += tile_length) {
            for (x = state->xoff; x < state->xsize; x += tile_width) {
                ...
                for (tile_y = 0; tile_y < min(tile_length, state->ysize - y);
                    ...
                    state->shuffle((UINT8*)im->image[tile_y + y] +
                                       x * im->pixelsize,
        ...
        // We could use TIFFStripSize, but for YCbCr data it returns subsampled
        // data size
        ...
        if (TIFFStripSize(tiff) > state->bytes) {
            // If the strip size as expected by LibTiff isn't what we're
            // expecting, abort. man:   TIFFStripSize returns the equivalent
            // size for a strip of data as it would be returned in a
...
ImagingLibTiffEncodeInit(ImagingCodecState state, char* filename, int fp)
{
    ...
    if (fp) {
        ...
        clientstate->tiff = TIFFClientOpen(filename,
                                           mode,
                                           (thandle_t)clientstate,
                                           _tiffReadProc,
                                           _tiffWriteProc,
...
int
ImagingLibTiffMergeFieldInfo(ImagingCodecState state,
...
int
ImagingLibTiffEncode(Imaging im,
                     ImagingCodecState state,
                     UINT8* buffer,
                     int bytes)
{
    /* One shot encoder. Encode everything to the tiff in the clientstate.
       ...
       If we're going to memory, then we need to write the whole file into
       memory, then parcel it back out to the pystring buffer bytes at a time.
    ...
    if (state->state == 0) {
        ...
        while (state->y < state->ysize) {
            ...
            if (TIFFWriteScanline(
                    tiff, (tdata_t)(state->buffer), (uint32)state->y, 0) ==
                -1) {

...
int ImagingZipDecode(Imaging im, ImagingCodecState state, UINT8* buf, Py_ssize_t bytes)
{
    ...
    while (context->z_stream.avail_in > 0) {
        ...
        case ZIP_PNG:
            ...
            case 3:
                ...
                for (; i <= row_len; i++)
                    state->buffer[i] += (state->buffer[i - bpp] + context->previous[i]) / 2;
                break;
            case 4:
                /* paeth filtering */
                bpp = (state->bits + 7) / 8;
                ...
                for (; i <= row_len; i++) {
                    ...
                    state->buffer[i] += (pa <= pb && pa <= pc) ? a : (pb <= pc) ? b : c;
                }
                break;
            default:
        ...
        if (context->interlaced) {
            ...
            if (state->bits >= 8) {
                ...
                for (i = 0; i < row_bits; i += state->bits) {
                    ...
                    byte <<= (i % 8);
                    state->shuffle((UINT8*)im->image[state->y] + col * im->pixelsize, &byte, 1);
                    col += COL_INCREMENT[context->pass];
                }
            }
            ...
            state->shuffle((UINT8*)im->image[state->y + state->yoff] + state->xoff * im->pixelsize,

...
int ImagingJpegEncode(Imaging im, ImagingCodecState state, UINT8* buf,
                      int bytes) {
    ...
    switch (state->state) {
        case 1:
            ...
            if (context->progressive) jpeg_simple_progression(&context->cinfo);
        ...
        case 5:
            ...
            /* Finish compression */
            if (context->destination.pub.free_in_buffer < 100) break;
...
const char* ImagingJpegVersion(void) {

...
static void
hash_to_list(const HashTable* h,
             const Pixel pixel,
...
static int
map_image_pixels_from_quantized_pixels(Pixel* pixelData,
                                       uint32_t nPixels,
                                       Pixel* paletteData,
                                       uint32_t nPaletteEntries,
                                       uint32_t* avgDist,
...
static int
map_image_pixels_from_median_box(Pixel* pixelData,
                                 uint32_t nPixels,
                                 Pixel* paletteData,
                                 uint32_t nPaletteEntries,
...
static int
compute_palette_from_median_cut(Pixel* pixelData,
                                uint32_t nPixels,
                                HashTable* medianBoxHash,
                                Pixel** palette,
                                uint32_t nPaletteEntries)
...
static int
recompute_palette_from_averages(Pixel* palette,
                                uint32_t nPaletteEntries,
                                uint32_t* avg[3],
                                uint32_t* count)
{
...
static int
compute_palette_from_quantized_pixels(Pixel* pixelData,
                                      uint32_t nPixels,
                                      Pixel* palette,
                                      uint32_t nPaletteEntries,
                                      uint32_t* avg[3],
        ...
        int threshold)
{
    ...
    while (1) {
        if (!built) {
            compute_palette_from_quantized_pixels(pixelData,
                                                  nPixels,
                                                  paletteData,
                                                  nPaletteEntries,
                                                  avg,
            ...
            recompute_palette_from_averages(
                paletteData, nPaletteEntries, avg, count);
         ...
         int kmeans)
{
    ...
    if (!compute_palette_from_median_cut(
            pixelData, nPixels, h, &p, nPaletteEntries)) {
        goto error_3;
    }
#ifndef NO_OUTPUT
    printf("done (%f)\n", (clock() - timer) / (double)CLOCKS_PER_SEC);
    ...
    if (!map_image_pixels_from_median_box(pixelData,
                                          nPixels,
                                          p,
                                          nPaletteEntries,
                                          h,
...
static void
compute_distances(const HashTable* h,
                  const Pixel pixel,
                  uint32_t* dist,
                  void* u)
{

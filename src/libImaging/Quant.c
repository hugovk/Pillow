...
static uint32_t unshifted_pixel_hash(const HashTable *h, const Pixel pixel) {
    return PIXEL_HASH(pixel.c.r, pixel.c.g, pixel.c.b);
}

static int unshifted_pixel_cmp(const HashTable *h, const Pixel pixel1,
                               const Pixel pixel2) {
...
static uint32_t pixel_hash(const HashTable *h, const Pixel pixel) {
    PixelHashData *d = (PixelHashData *)hashtable_get_user_data(h);
    return PIXEL_HASH(pixel.c.r >> d->scale, pixel.c.g >> d->scale,
                      pixel.c.b >> d->scale);
}

static int pixel_cmp(const HashTable *h, const Pixel pixel1,
                     const Pixel pixel2) {
    PixelHashData *d = (PixelHashData *)hashtable_get_user_data(h);
    uint32_t A, B;
    A = PIXEL_HASH(pixel1.c.r >> d->scale, pixel1.c.g >> d->scale,
                   pixel1.c.b >> d->scale);
    B = PIXEL_HASH(pixel2.c.r >> d->scale, pixel2.c.g >> d->scale,
                   pixel2.c.b >> d->scale);
    return (A == B) ? 0 : ((A < B) ? -1 : 1);
}

static void exists_count_func(const HashTable *h, const Pixel key,
                              uint32_t *val) {
    *val += 1;
}

static void new_count_func(const HashTable *h, const Pixel key, uint32_t *val) {
    *val = 1;
}

static void rehash_collide(const HashTable *h, Pixel *keyp, uint32_t *valp,
                           Pixel newkey, uint32_t newval) {
    *valp += newval;
}

/* %% */

static HashTable *create_pixel_hash(Pixel *pixelData, uint32_t nPixels) {
    PixelHashData *d;
    HashTable *hash;
    uint32_t i;
#ifndef NO_OUTPUT
...
static void destroy_pixel_hash(HashTable *hash) {
...
static int compute_box_volume(BoxNode *b) {
...
static void hash_to_list(const HashTable *h, const Pixel pixel,
                         const uint32_t count, void *u) {
    PixelHashData *d = (PixelHashData *)hashtable_get_user_data(h);
...
static PixelList *mergesort_pixels(PixelList *head, int i) {
    PixelList *c, *t, *a, *b, *p;
    if (!head || !head->next[i]) {
        if (head) {
            head->next[i] = NULL;
...
static int test_sorted(PixelList *pl[3]) {
    int i, n, l;
    PixelList *t;
...
static int box_heap_cmp(const Heap *h, const void *A, const void *B) {
    BoxNode *a = (BoxNode *)A;
    BoxNode *b = (BoxNode *)B;
...
static int splitlists(PixelList *h[3], PixelList *t[3], PixelList *nh[2][3],
...
#endif
    node->axis = axis;
    if (!splitlists(node->head, node->tail, heads, tails, newCounts, axis,
                    node->pixelCount)) {
#ifndef NO_OUTPUT
...
static BoxNode *median_cut(PixelList *hl[3], uint32_t imPixelCount,
                           int nPixels) {
    PixelList *tl[3];
    int i;
    BoxNode *root;
...
static void free_box_tree(BoxNode *n) {
    PixelList *p, *pp;
...
static int checkContained(BoxNode *n, Pixel *pp) {
    if (n->l && n->r) {
...
static int annotate_hash_table(BoxNode *n, HashTable *h, uint32_t *box) {
    PixelList *p;
    PixelHashData *d = (PixelHashData *)hashtable_get_user_data(h);
    Pixel q;
    if (n->l && n->r) {
...
static int _sort_ulong_ptr_keys(const void *a, const void *b) {
    uint32_t A = **(uint32_t **)a;
    uint32_t B = **(uint32_t **)b;
    return (A == B) ? 0 : ((A < B) ? -1 : +1);
}

static int resort_distance_tables(uint32_t *avgDist, uint32_t **avgDistSortKey,
                                  Pixel *p, uint32_t nEntries) {
    uint32_t i, j, k;
    uint32_t **skRow;
    uint32_t *skElt;
...
static int build_distance_tables(uint32_t *avgDist, uint32_t **avgDistSortKey,
                                 Pixel *p, uint32_t nEntries) {
    uint32_t i, j;
    ...
    for (i = 0; i < nEntries; i++) {
        qsort(avgDistSortKey + i * nEntries, nEntries, sizeof(uint32_t *),
              _sort_ulong_ptr_keys);
    }
    return 1;
}

static int map_image_pixels(Pixel *pixelData, uint32_t nPixels,
...
static int map_image_pixels_from_quantized_pixels(
...
static int map_image_pixels_from_median_box(
    Pixel *pixelData, uint32_t nPixels, Pixel *paletteData,
    uint32_t nPaletteEntries, HashTable *medianBoxHash, uint32_t *avgDist,
...
static int compute_palette_from_median_cut(Pixel *pixelData, uint32_t nPixels,
...
static int recompute_palette_from_averages(Pixel *palette,
                                           uint32_t nPaletteEntries,
...
static int compute_palette_from_quantized_pixels(
    Pixel *pixelData, uint32_t nPixels, Pixel *palette,
    uint32_t nPaletteEntries, uint32_t *avg[3], uint32_t *count, uint32_t *qp) {
    uint32_t i;
...
static int k_means(Pixel *pixelData, uint32_t nPixels, Pixel *paletteData,
                   uint32_t nPaletteEntries, uint32_t *qp, int threshold) {
    uint32_t *avg[3];
    uint32_t *count;
    uint32_t i;
    uint32_t *avgDist;
...
int quantize(Pixel *pixelData, uint32_t nPixels, uint32_t nQuantPixels,
             Pixel **palette, uint32_t *paletteLength,
             uint32_t **quantizedPixels, int kmeans) {
    PixelList *hl[3];
    HashTable *h;
    BoxNode *root;
    uint32_t i;
...
#include <math.h>
    {
        ...
        for (i = 0; i < nPixels; i++) {
            ...
            if (qp[i] != bestmatch) {
                printf(
                    "discrepancy in matching algorithms pixel %d [%d %d] %f "
                    "%f\n",
                    i, qp[i], bestmatch,
...
static void compute_distances(const HashTable *h, const Pixel pixel,
                              uint32_t *dist, void *u) {
...
int quantize2(Pixel *pixelData, uint32_t nPixels, uint32_t nQuantPixels,
              Pixel **palette, uint32_t *paletteLength,
              uint32_t **quantizedPixels, int kmeans) {
    HashTable *h;
    uint32_t i;
...
Imaging ImagingQuantize(Imaging im, int colors, int mode, int kmeans) {
    int i, j;
    int x, y, v;
    UINT8 *pp;
    Pixel *p;
    ...
    if (!p) return ImagingError_MemoryError();
    ...
    if (!strcmp(im->mode, "L")) {
        ...
        for (i = y = 0; y < im->ysize; y++)
            for (x = 0; x < im->xsize; x++, i++) p[i].v = im->image32[y][x];
    ...
    switch (mode) {
        case 0:
            /* median cut */
            result = quantize(p, im->xsize * im->ysize, colors, &palette,
                              &paletteLength, &newData, kmeans);
            break;
        case 1:
            /* maximum coverage */
            result = quantize2(p, im->xsize * im->ysize, colors, &palette,
                               &paletteLength, &newData, kmeans);
            break;
        case 2:

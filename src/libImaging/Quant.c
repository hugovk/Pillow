...
#define _DISTSQR(p1, p2) \
    _SQR((int)((p1)->c.r) - (int)((p2)->c.r)) + _SQR((int)((p1)->c.g) - (int)((p2)->c.g)) + _SQR((int)((p1)->c.b) - (int)((p2)->c.b))

#define MAX_HASH_ENTRIES 65536

#define PIXEL_HASH(r, g, b) \
    (((unsigned int)(r)) * 463 ^ ((unsigned int)(g) << 8) * 10069 ^ ((unsigned int)(b) << 16) * 64997)
    ...
    uint32_t pixelCount)
{
...
#ifdef TEST_SPLIT
    {
        ...
        for (_i = 1; _i < 3; _i++) {
            if (_prevCount[_i] != _prevCount[_i - 1] || _nextCount[_i] != _nextCount[_i - 1] || _prevCount[_i] != _nextCount[_i]) {
                printf("{%d %d %d} {%d %d %d}\n",
                    _prevCount[0],
                    _prevCount[1],
                    _prevCount[2],
...
checkContained(BoxNode* n, Pixel* pp)
{
    ...
    if (
        pp->c.r <= n->head[0]->p.c.r && pp->c.r >= n->tail[0]->p.c.r && pp->c.g <= n->head[1]->p.c.g && pp->c.g >= n->tail[1]->p.c.g && pp->c.b <= n->head[2]->p.c.b && pp->c.b >= n->tail[2]->p.c.b) {
        return 1;
    }
    return 0;
}
    ...
    uint32_t nEntries)
{
    ...
    for (i = 0; i < nEntries; i++) {
        ...
        for (j = 0; j < i; j++) {
            avgDist[j * nEntries + i] = avgDist[i * nEntries + j] = _DISTSQR(p + i, p + j);
        }
    }
    for (i = 0; i < nEntries; i++) {
...
#include <math.h>
    {
        ...
        for (i = 0; i < nPixels; i++) {
            if (hashtable_lookup(h2, pixelData[i], &paletteEntry)) {
                ...
                bestmatch = 0;
                bestdist = _SQR(pixelData[i].c.r - p[0].c.r) + _SQR(pixelData[i].c.g - p[0].c.g) + _SQR(pixelData[i].c.b - p[0].c.b);
                for (j = 1; j < nPaletteEntries; j++) {
                    dist = _SQR(pixelData[i].c.r - p[j].c.r) + _SQR(pixelData[i].c.g - p[j].c.g) + _SQR(pixelData[i].c.b - p[j].c.b);
            ...
            if (qp[i] != bestmatch) {
                printf("discrepancy in matching algorithms pixel %d [%d %d] %f %f\n",
                    i, qp[i], bestmatch,
                    sqrt((double)(_SQR(pixelData[i].c.r - p[qp[i]].c.r) + _SQR(pixelData[i].c.g - p[qp[i]].c.g) + _SQR(pixelData[i].c.b - p[qp[i]].c.b))),
                    sqrt((double)(_SQR(pixelData[i].c.r - p[bestmatch].c.r) + _SQR(pixelData[i].c.g - p[bestmatch].c.g) + _SQR(pixelData[i].c.b - p[bestmatch].c.b))));
            }
        }
        hashtable_free(h2);
    }
...
ImagingQuantize(Imaging im, int colors, int mode, int kmeans)
{
    ...
    if (strcmp(im->mode, "L") != 0 && strcmp(im->mode, "P") != 0 && strcmp(im->mode, "RGB") != 0 && strcmp(im->mode, "RGBA") != 0)
        return ImagingError_ModeError();

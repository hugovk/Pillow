    ...
    int filterid, int fill)
{
    ...
    for (y = y0; y < y1; y++) {
        ...
        for (x = x0; x < x1; x++) {
            if (!transform(&xx, &yy, x - x0, y - y0, transform_data) || !filter(out, imIn, xx, yy)) {
                if (fill)
...
{
    return (fabs(x * a[0] + y * a[1] + a[2]) < 32768.0 && fabs(x * a[3] + y * a[4] + a[5]) < 32768.0);
}
    ...
    double a[6], int filterid, int fill)
{
    ...
    if (check_fixed(a, 0, 0) && check_fixed(a, x1 - x0, y1 - y0) && check_fixed(a, 0, y1 - y0) && check_fixed(a, x1 - x0, 0))

...
Imaging ImagingFlipLeftRight(Imaging imOut, Imaging imIn) {
...
#define FLIP_LEFT_RIGHT(INT, image)                              \
    for (y = 0; y < imIn->ysize; y++) {                          \
        ...
        xr = imIn->xsize - 1;                                    \
        for (x = 0; x < imIn->xsize; x++, xr--) out[xr] = in[x]; \
    }
...
Imaging ImagingFlipTopBottom(Imaging imOut, Imaging imIn) {
    ImagingSectionCookie cookie;
    int y, yr;
...
Imaging ImagingRotate90(Imaging imOut, Imaging imIn) {
    ImagingSectionCookie cookie;
...
Imaging ImagingTranspose(Imaging imOut, Imaging imIn) {
    ImagingSectionCookie cookie;
...
Imaging ImagingTransverse(Imaging imOut, Imaging imIn) {
    ImagingSectionCookie cookie;
...
Imaging ImagingRotate180(Imaging imOut, Imaging imIn) {
    ImagingSectionCookie cookie;
    int x, y, xr, yr;
...
Imaging ImagingRotate270(Imaging imOut, Imaging imIn) {
    ImagingSectionCookie cookie;
...
static int affine_transform(double* xout, double* yout, int x, int y,
                            void* data) {
...
static int perspective_transform(double* xout, double* yout, int x, int y,
                                 void* data) {
    double* a = (double*)data;
    double a0 = a[0];
    double a1 = a[1];
...
static int quad_transform(double* xout, double* yout, int x, int y,
                          void* data) {
...
static int nearest_filter8(void* out, Imaging im, double xin, double yin) {
    int x = COORD(xin);
    int y = COORD(yin);
    if (x < 0 || x >= im->xsize || y < 0 || y >= im->ysize) return 0;
    ((UINT8*)out)[0] = im->image8[y][x];
    return 1;
}

static int nearest_filter16(void* out, Imaging im, double xin, double yin) {
    int x = COORD(xin);
    int y = COORD(yin);
    if (x < 0 || x >= im->xsize || y < 0 || y >= im->ysize) return 0;
...
static int nearest_filter32(void* out, Imaging im, double xin, double yin) {
    int x = COORD(xin);
    int y = COORD(yin);
    if (x < 0 || x >= im->xsize || y < 0 || y >= im->ysize) return 0;
...
static int bilinear_filter8(void* out, Imaging im, double xin, double yin) {
    BILINEAR_HEAD(UINT8);
...
static int bilinear_filter32I(void* out, Imaging im, double xin, double yin) {
    INT32 k;
    BILINEAR_HEAD(INT32);
...
static int bilinear_filter32F(void* out, Imaging im, double xin, double yin) {
    FLOAT32 k;
...
static int bilinear_filter32LA(void* out, Imaging im, double xin, double yin) {
    BILINEAR_HEAD(UINT8);
...
static int bilinear_filter32RGB(void* out, Imaging im, double xin, double yin) {
    int b;
    BILINEAR_HEAD(UINT8);
...
static int bicubic_filter8(void* out, Imaging im, double xin, double yin) {
    BICUBIC_HEAD(UINT8);
...
static int bicubic_filter32I(void* out, Imaging im, double xin, double yin) {
    INT32 k;
    BICUBIC_HEAD(INT32);
...
static int bicubic_filter32F(void* out, Imaging im, double xin, double yin) {
    FLOAT32 k;
...
static int bicubic_filter32LA(void* out, Imaging im, double xin, double yin) {
    BICUBIC_HEAD(UINT8);
...
static int bicubic_filter32RGB(void* out, Imaging im, double xin, double yin) {
    int b;
    BICUBIC_HEAD(UINT8);
    for (b = 0; b < im->bands; b++) {
...
static ImagingTransformFilter getfilter(Imaging im, int filterid) {
    switch (filterid) {
        case IMAGING_TRANSFORM_NEAREST:
            if (im->image8) switch (im->type) {
...
Imaging ImagingGenericTransform(Imaging imOut, Imaging imIn, int x0, int y0,
                                int x1, int y1, ImagingTransformMap transform,
    ...
    if (!filter) return (Imaging)ImagingError_ValueError("bad filter number");
    ...
    if (x0 < 0) x0 = 0;
    if (y0 < 0) y0 = 0;
    if (x1 > imOut->xsize) x1 = imOut->xsize;
    if (y1 > imOut->ysize) y1 = imOut->ysize;
    ...
    for (y = y0; y < y1; y++) {
        ...
        for (x = x0; x < x1; x++) {
            if (!transform(&xx, &yy, x - x0, y - y0, transform_data) ||
                ...
                if (fill) memset(out, 0, imOut->pixelsize);
            }
            out += imOut->pixelsize;
        }
    }
...
static Imaging ImagingScaleAffine(Imaging imOut, Imaging imIn, int x0, int y0,
                                  int x1, int y1, double a[6], int fill) {
...
#define AFFINE_SCALE(pixel, image)                                           \
    for (y = y0; y < y1; y++) {                                              \
        ...
        pixel *in, *out;                                                     \
        out = imOut->image[y];                                               \
        if (fill && x1 > x0) memset(out + x0, 0, (x1 - x0) * sizeof(pixel)); \
        if (yi >= 0 && yi < imIn->ysize) {                                   \
            in = imIn->image[yi];                                            \
            for (x = xmin; x < xmax; x++) out[x] = in[xintab[x]];            \
        }                                                                    \
        yo += a[4];                                                          \
    }
...
static inline int check_fixed(double a[6], int x, int y) {
    return (fabs(x * a[0] + y * a[1] + a[2]) < 32768.0 &&
            fabs(x * a[3] + y * a[4] + a[5]) < 32768.0);
}

static inline Imaging affine_fixed(Imaging imOut, Imaging imIn, int x0, int y0,
                                   int x1, int y1, double a[6], int filterid,
                                   int fill) {
...
#define AFFINE_TRANSFORM_FIXED(pixel, image)                                 \
    for (y = y0; y < y1; y++) {                                              \
        ...
        for (x = x0; x < x1; x++, out++) {                                   \
            ...
            if (xin >= 0 && xin < xsize) {                                   \
                yin = yy >> 16;                                              \
                if (yin >= 0 && yin < ysize) *out = imIn->image[yin][xin];   \
            }                                                                \
            xx += a0;                                                        \
            yy += a3;                                                        \
        }                                                                    \
...
Imaging ImagingTransformAffine(Imaging imOut, Imaging imIn, int x0, int y0,
                               int x1, int y1, double a[6], int filterid,
                               int fill) {
...
Imaging ImagingTransform(Imaging imOut, Imaging imIn, int method, int x0,
                         int y0, int x1, int y1, double a[8], int filterid,
                         int fill) {

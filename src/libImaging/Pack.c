...
static void pack1(UINT8* out, const UINT8* in, int pixels) {
    int i, m, b;
    /* bilevel (black is 0) */
    b = 0;
    m = 128;
...
static void pack1I(UINT8* out, const UINT8* in, int pixels) {
    int i, m, b;
    /* bilevel (black is 1) */
    b = 0;
    m = 128;
...
static void pack1R(UINT8* out, const UINT8* in, int pixels) {
    int i, m, b;
...
static void pack1IR(UINT8* out, const UINT8* in, int pixels) {
    int i, m, b;
...
static void pack1L(UINT8* out, const UINT8* in, int pixels) {
    int i;
    /* bilevel, stored as bytes */
    for (i = 0; i < pixels; i++) out[i] = (in[i] != 0) ? 255 : 0;
}

static void packP4(UINT8* out, const UINT8* in, int pixels) {
    while (pixels >= 2) {
        *out++ = (in[0] << 4) | (in[1] & 15);
        in += 2;
        pixels -= 2;
    ...
    if (pixels) out[0] = (in[0] << 4);
}

static void packP2(UINT8* out, const UINT8* in, int pixels) {
    while (pixels >= 4) {
...
static void packL16(UINT8* out, const UINT8* in, int pixels) {
    int i;
...
static void packL16B(UINT8* out, const UINT8* in, int pixels) {
    int i;
...
static void packLA(UINT8* out, const UINT8* in, int pixels) {
    int i;
...
static void packLAL(UINT8* out, const UINT8* in, int pixels) {
    int i;
    /* LA, line interleaved */
    for (i = 0; i < pixels; i++) {
        out[i] = in[R];
...
void ImagingPackRGB(UINT8* out, const UINT8* in, int pixels) {
    int i = 0;
    /* RGB triplets */
#ifdef __sparc
...
void ImagingPackXRGB(UINT8* out, const UINT8* in, int pixels) {
    int i;
...
void ImagingPackBGR(UINT8* out, const UINT8* in, int pixels) {
    int i;
    /* RGB, reversed bytes */
    for (i = 0; i < pixels; i++) {
        out[0] = in[B];
...
void ImagingPackBGRX(UINT8* out, const UINT8* in, int pixels) {
    int i;
...
void ImagingPackXBGR(UINT8* out, const UINT8* in, int pixels) {
    int i;
...
void ImagingPackBGRA(UINT8* out, const UINT8* in, int pixels) {
    int i;
    /* BGRX, reversed bytes with right padding */
    for (i = 0; i < pixels; i++) {
        out[0] = in[B];
...
void ImagingPackABGR(UINT8* out, const UINT8* in, int pixels) {
    int i;
    /* XBGR, reversed bytes with left padding */
    for (i = 0; i < pixels; i++) {
        out[0] = in[A];
...
static void packRGBL(UINT8* out, const UINT8* in, int pixels) {
    int i;
...
static void packRGBXL(UINT8* out, const UINT8* in, int pixels) {
    int i;
...
static void packI16B(UINT8* out, const UINT8* in_, int pixels) {
    int i;
    UINT16 tmp_;
...
static void packI16N_I16B(UINT8* out, const UINT8* in, int pixels) {
    int i;
    UINT8* tmp = (UINT8*)in;
    for (i = 0; i < pixels; i++) {
        C16B;
...
static void packI16N_I16(UINT8* out, const UINT8* in, int pixels) {
    int i;
    UINT8* tmp = (UINT8*)in;
    for (i = 0; i < pixels; i++) {
        C16L;
...
static void packI32S(UINT8* out, const UINT8* in, int pixels) {
    int i;
    UINT8* tmp = (UINT8*)in;
    for (i = 0; i < pixels; i++) {
        C32L;
...
void ImagingPackLAB(UINT8* out, const UINT8* in, int pixels) {
    int i;
    /* LAB triplets */
    for (i = 0; i < pixels; i++) {
        out[0] = in[0];
...
static void copy3(UINT8* out, const UINT8* in, int pixels) {
    /* BGR;24, etc */
...
ImagingShuffler ImagingFindPacker(const char* mode, const char* rawmode,
                                  int* bits_out) {
    int i;
    ...
    for (i = 0; packers[i].rawmode; i++)
        if (strcmp(packers[i].mode, mode) == 0 &&
            ...
            if (bits_out) *bits_out = packers[i].bits;

...
int ImagingRawEncode(Imaging im, ImagingCodecState state, UINT8* buf, int bytes)
{
    UINT8* ptr;
    ...
    while (bytes >= state->bytes) {
        ...
        state->shuffle(ptr, (UINT8*)im->image[state->y + state->yoff] + state->xoff * im->pixelsize, state->xsize);

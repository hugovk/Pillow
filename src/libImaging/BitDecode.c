...
int ImagingBitDecode(Imaging im, ImagingCodecState state, UINT8* buf,
                     Py_ssize_t bytes) {
    ...
    if (state->state == 0) {
        ...
        if (bitstate->sign) bitstate->signmask = (1 << (bitstate->bits - 1));
    ...
    while (bytes > 0) {
        ...
        if (bitstate->fill & 1) /* fill MSB first */
        ...
        while (bitstate->bitcount >= bitstate->bits) {
            ...
            *(FLOAT32*)(&im->image32[state->y][state->x]) = pixel;
            ...
            if (++state->x >= state->xsize) {
                ...
                state->x = 0;
                /* reset bit buffer */
                if (bitstate->pad > 0) bitstate->bitcount = 0;
            }
        }
    }

...
int precompute_coeffs(int inSize,
                      float in0,
                      float in1,
                      int outSize,
...
void ImagingResampleHorizontal_8bpc(Imaging imOut,
                                    Imaging imIn,
                                    int offset,
                                    int ksize,
                                    int* bounds,
            ...
            for (yy = 0; yy < imOut->ysize; yy++) {
                for (xx = 0; xx < imOut->xsize; xx++) {
                    ...
                    for (x = 0; x < xmax; x++) {
                        ss0 +=
                            ((UINT8)
                                 imIn->image[yy + offset][(x + xmin) * 4 + 0]) *
                            k[x];
                        ss3 +=
                            ((UINT8)
                                 imIn->image[yy + offset][(x + xmin) * 4 + 3]) *
                            k[x];
                    }
            ...
            for (yy = 0; yy < imOut->ysize; yy++) {
                for (xx = 0; xx < imOut->xsize; xx++) {
                    ...
                    for (x = 0; x < xmax; x++) {
                        ...
                        ss1 +=
                            ((UINT8)
                                 imIn->image[yy + offset][(x + xmin) * 4 + 1]) *
                            k[x];
                        ss2 +=
                            ((UINT8)
                                 imIn->image[yy + offset][(x + xmin) * 4 + 2]) *
                            k[x];
                    }
...
void ImagingResampleVertical_8bpc(Imaging imOut,
                                  Imaging imIn,
                                  int offset,
                                  int ksize,
                                  int* bounds,
...
void ImagingResampleHorizontal_32bpc(Imaging imOut,
                                     Imaging imIn,
                                     int offset,
                                     int ksize,
                                     int* bounds,
        ...
        case IMAGING_TYPE_INT32:
            for (yy = 0; yy < imOut->ysize; yy++) {
                for (xx = 0; xx < imOut->xsize; xx++) {
                    ...
                    for (x = 0; x < xmax; x++)
                        ss +=
                            IMAGING_PIXEL_I(imIn, x + xmin, yy + offset) * k[x];
        ...
        case IMAGING_TYPE_FLOAT32:
            for (yy = 0; yy < imOut->ysize; yy++) {
                for (xx = 0; xx < imOut->xsize; xx++) {
                    ...
                    for (x = 0; x < xmax; x++)
                        ss +=
                            IMAGING_PIXEL_F(imIn, x + xmin, yy + offset) * k[x];
...
void ImagingResampleVertical_32bpc(Imaging imOut,
                                   Imaging imIn,
                                   int offset,
                                   int ksize,
                                   int* bounds,
...
typedef void (*ResampleFunction)(Imaging imOut,
                                 Imaging imIn,
                                 int offset,
                                 int ksize,
                                 int* bounds,
...
Imaging ImagingResampleInner(Imaging imIn,
                             int xsize,
                             int ysize,
                             struct filter* filterp,
                             float box[4],
...
Imaging ImagingResample(Imaging imIn,
                        int xsize,
                        int ysize,
                        int filter,
                        float box[4]) {

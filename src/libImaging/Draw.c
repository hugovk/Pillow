...
hline8(Imaging im, int x0, int y0, int x1, int ink)
{
    ...
    if (y0 >= 0 && y0 < im->ysize) {
        ...
        if (x0 <= x1) {
            ...
            memset(im->image8[y0] + x0 * pixelwidth,
                   (UINT8)ink,
                   (x1 - x0 + 1) * pixelwidth);
        }
    }
...
static void
draw_horizontal_lines(Imaging im,
                      int n,
                      Edge* e,
                      int ink,
                      int* x_pos,
...
int
ImagingDrawWideLine(Imaging im,
                    int x0,
                    int y0,
                    int x1,
                    int y1,
...
int
ImagingDrawRectangle(Imaging im,
                     int x0,
                     int y0,
                     int x1,
                     int y1,
        ...
        if (end - start < 360) {
            ...
            if (mode == PIESLICE) {
                if (x != cx || y != cy) {
                    ...
                    if (inner) {
                        ImagingDrawWideLine(
                            im, sx, sy, cx, cy, &ink, width, op);
                        ImagingDrawWideLine(
                            im, cx, cy, lx, ly, &ink, width, op);
                    }
                }
            } else if (mode == CHORD) {
...
int
ImagingDrawPieslice(Imaging im,
                    int x0,
                    int y0,
                    int x1,
                    int y1,
...
{
    return ellipse(
        im, x0, y0, x1, y1, start, end, ink, fill, width, PIESLICE, op);
}
...
int
ImagingOutlineCurve(ImagingOutline outline,
                    float x1,
                    float y1,
                    float x2,
                    float y2,

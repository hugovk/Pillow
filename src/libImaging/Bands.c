...
Imaging ImagingGetBand(Imaging imIn, int band) {
    Imaging imOut;
    int x, y;
    ...
    /* Shortcuts */
    if (imIn->bands == 1) return ImagingCopy(imIn);

    /* Special case for LXXA etc */
    if (imIn->bands == 2 && band == 1) band = 3;
...
Imaging ImagingPutBand(Imaging imOut, Imaging imIn, int band) {
    int x, y;
    ...
    /* Shortcuts */
    if (imOut->bands == 1) return ImagingCopy2(imOut, imIn);

    /* Special case for LXXA etc */
    if (imOut->bands == 2 && band == 1) band = 3;
...
Imaging ImagingFillBand(Imaging imOut, int band, int color) {
    int x, y;
...
Imaging ImagingMerge(const char* mode, Imaging bands[4]) {
    int i, x, y;
    int bandsCount = 0;
    Imaging imOut;
    Imaging firstBand;
    ...
    if (imOut->bands == 1) return ImagingCopy2(imOut, firstBand);

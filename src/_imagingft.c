...
static size_t
text_layout_fallback(PyObject* string,
                     FontObject* self,
                     const char* dir,
                     PyObject* features,
                     const char* lang,
            ...
            int mask)
{
    ...
    if (p_raqm.raqm && self->layout_engine == LAYOUT_RAQM) {
        count = text_layout_raqm(
            string, self, dir, features, lang, glyph_info, mask);
    } else {
...
font_getsize(FontObject* self, PyObject* args)
{
    ...
    PyObject* string;
    if (!PyArg_ParseTuple(
            args, "O|zOz:getsize", &string, &dir, &features, &lang))
        return NULL;
        ...
        if (stroker != NULL) {
            ...
            if (!error) {
                ...
                error = FT_Glyph_To_Bitmap(
                    &glyph, FT_RENDER_MODE_NORMAL, &origin, 1);
            }
            if (error) {
                return geterror(error);
            }
...
font_getvaraxes(FontObject* self)
{
    ...
    for (i = 0; i < num_axis; i++) {
        ...
        PyDict_SetItemString(
            list_axis, "default", PyLong_FromLong(axis.def / 65536));
        PyDict_SetItemString(

...
typedef bool (*t_raqm_version_atleast)(unsigned int major, unsigned int minor,
                                       unsigned int micro);
...
typedef bool (*t_raqm_set_language)(raqm_t* rq, const char* lang, size_t start,
                                    size_t len);
typedef bool (*t_raqm_add_font_feature)(raqm_t* rq, const char* feature,
                                        int len);
...
static PyObject* geterror(int code) {
    int i;
...
static int setraqm(void) {
    ...
    p_raqm.set_language =
        (t_raqm_set_language)dlsym(p_raqm.raqm, "raqm_set_language");
    ...
    if (dlsym(p_raqm.raqm, "raqm_index_to_position")) {
        p_raqm.get_glyphs =
            (t_raqm_get_glyphs)dlsym(p_raqm.raqm, "raqm_get_glyphs");
        p_raqm.version = 2;
    } else {
        p_raqm.version = 1;
    ...
    if (dlerror() ||
        !(p_raqm.create && p_raqm.set_text && p_raqm.set_text_utf8 &&
          p_raqm.set_par_direction && p_raqm.set_language &&
          p_raqm.add_font_feature && p_raqm.set_freetype_face &&
          p_raqm.layout && (p_raqm.get_glyphs || p_raqm.get_glyphs_01) &&
          p_raqm.destroy)) {
        dlclose(p_raqm.raqm);
        p_raqm.raqm = NULL;
        return 2;
...
#else
    p_raqm.version_atleast = (t_raqm_version_atleast)GetProcAddress(
        p_raqm.raqm, "raqm_version_atleast");
    p_raqm.create = (t_raqm_create)GetProcAddress(p_raqm.raqm, "raqm_create");
    p_raqm.set_text =
        (t_raqm_set_text)GetProcAddress(p_raqm.raqm, "raqm_set_text");
    p_raqm.set_text_utf8 =
        (t_raqm_set_text_utf8)GetProcAddress(p_raqm.raqm, "raqm_set_text_utf8");
    p_raqm.set_par_direction = (t_raqm_set_par_direction)GetProcAddress(
        p_raqm.raqm, "raqm_set_par_direction");
    p_raqm.set_language =
        (t_raqm_set_language)GetProcAddress(p_raqm.raqm, "raqm_set_language");
    p_raqm.add_font_feature = (t_raqm_add_font_feature)GetProcAddress(
        p_raqm.raqm, "raqm_add_font_feature");
    p_raqm.set_freetype_face = (t_raqm_set_freetype_face)GetProcAddress(
        p_raqm.raqm, "raqm_set_freetype_face");
    p_raqm.layout = (t_raqm_layout)GetProcAddress(p_raqm.raqm, "raqm_layout");
    p_raqm.destroy =
        (t_raqm_destroy)GetProcAddress(p_raqm.raqm, "raqm_destroy");
    if (GetProcAddress(p_raqm.raqm, "raqm_index_to_position")) {
        ...
        p_raqm.version = 1;
        p_raqm.get_glyphs_01 = (t_raqm_get_glyphs_01)GetProcAddress(
            p_raqm.raqm, "raqm_get_glyphs");
    }
...
static PyObject* getfont(PyObject* self_, PyObject* args, PyObject* kw) {
    ...
    static char* kwlist[] = {"filename",   "size",          "index", "encoding",
                             "font_bytes", "layout_engine", NULL};
    ...
    if (!error) error = FT_Set_Pixel_Sizes(self->face, 0, size);
...
static int font_getchar(PyObject* string, int index, FT_ULong* char_out) {
    if (PyUnicode_Check(string)) {
        if (index >= PyUnicode_GET_LENGTH(string)) return 0;
...
static size_t text_layout_raqm(PyObject* string, FontObject* self,
                               const char* dir, PyObject* features,
                               const char* lang, GlyphInfo** glyph_info,
                               int mask) {
    ...
    if (dir) {
        if (strcmp(dir, "rtl") == 0)
            ...
            if (p_raqm.version_atleast == NULL ||
                !(*p_raqm.version_atleast)(0, 7, 0)) {
                PyErr_SetString(
                    PyExc_ValueError,
...
static size_t text_layout_fallback(PyObject* string, FontObject* self,
                                   const char* dir, PyObject* features,
                                   const char* lang, GlyphInfo** glyph_info,
                                   int mask) {
    ...
    if (features != Py_None || dir != NULL || lang != NULL) {
        PyErr_SetString(PyExc_KeyError,
                        "setting text direction, language or font features is "
...
static size_t text_layout(PyObject* string, FontObject* self, const char* dir,
                          PyObject* features, const char* lang,
                          GlyphInfo** glyph_info, int mask) {
    size_t count;
    ...
    if (p_raqm.raqm && self->layout_engine == LAYOUT_RAQM) {
        ...
        count = text_layout_fallback(string, self, dir, features, lang,
                                     glyph_info, mask);
    }
    return count;
}

static PyObject* font_getsize(FontObject* self, PyObject* args) {
    ...
    PyObject* string;
    if (!PyArg_ParseTuple(args, "O|zOz:getsize", &string, &dir, &features,
                          &lang))
        return NULL;
    ...
    for (i = 0; i < count; i++) {
        ...
        if (horizontal_dir) {
            ...
            offset = glyph_info[i].x_advance - face->glyph->metrics.width -
                     face->glyph->metrics.horiBearingX;
            if (offset < 0) x_advanced -= offset;
            if (x_advanced > x_max) x_max = x_advanced;
            ...
            if (i == count - 1) {
                ...
                if (offset < 0) y_max -= offset;
            }

            if (bbox.xMax > x_max) x_max = bbox.xMax;
            if (i == 0 || bbox.xMin < x_min) x_min = bbox.xMin;
        }
    ...
    return Py_BuildValue("(ii)(ii)", PIXEL(x_max - x_min), PIXEL(y_max - y_min),
                         PIXEL(xoffset), yoffset);
}

static PyObject* font_render(FontObject* self, PyObject* args) {
    int x;
    unsigned int y;
    Imaging im;
    ...
    if (!PyArg_ParseTuple(args, "On|izOzi:render", &string, &id, &mask, &dir,
                          &features, &lang, &stroke_width)) {
        return NULL;
    }
    ...
    if (stroke_width) {
        ...
        FT_Stroker_Set(stroker, (FT_Fixed)stroke_width * 64,
    ...
    for (i = 0; i < count; i++) {
        ...
        if (temp > ascender) ascender = temp;
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        for (bitmap_y = 0; bitmap_y < bitmap.rows; bitmap_y++) {
            if (horizontal_dir) {
                ...
                yy = bitmap_y + PIXEL(y + glyph_slot->metrics.vertBearingY) +
                     ascender;
            ...
            if (yy >= 0 && yy < im->ysize) {
                ...
                if (mask) {
                    ...
                    int j, k, m = 128;
                    for (j = k = 0; j < x1; j++) {
                        if (j >= x0 && (source[k] & m)) target[j] = 255;
                        if (!(m >>= 1)) {
                            m = 128;
                            k++;
                        }
...
static PyObject* font_getvarnames(FontObject* self) {
    int error;
    ...
    for (i = 0; i < name_count; i++) {
        ...
        for (j = 0; j < num_namedstyles; j++) {
            if (PyList_GetItem(list_names, j) != NULL) continue;
...
static PyObject* font_getvaraxes(FontObject* self) {
    int error;
...
static PyObject* font_setvarname(FontObject* self, PyObject* args) {
    int error;

    int instance_index;
    if (!PyArg_ParseTuple(args, "i", &instance_index)) return NULL;
...
static PyObject* font_setvaraxes(FontObject* self, PyObject* args) {
    int error;
    ...
    FT_Fixed* coords;
    FT_Fixed coord;
    if (!PyArg_ParseTuple(args, "O", &axes)) return NULL;
...
static PyObject* font_getattr_family(FontObject* self, void* closure) {
    if (self->face->family_name)
        return PyUnicode_FromString(self->face->family_name);
    Py_RETURN_NONE;
}

static PyObject* font_getattr_style(FontObject* self, void* closure) {
    if (self->face->style_name)
        return PyUnicode_FromString(self->face->style_name);
    Py_RETURN_NONE;
}

static PyObject* font_getattr_ascent(FontObject* self, void* closure) {
    return PyLong_FromLong(PIXEL(self->face->size->metrics.ascender));
}

static PyObject* font_getattr_descent(FontObject* self, void* closure) {
    return PyLong_FromLong(-PIXEL(self->face->size->metrics.descender));
}

static PyObject* font_getattr_height(FontObject* self, void* closure) {
    return PyLong_FromLong(PIXEL(self->face->size->metrics.height));
}

static PyObject* font_getattr_x_ppem(FontObject* self, void* closure) {
    return PyLong_FromLong(self->face->size->metrics.x_ppem);
}

static PyObject* font_getattr_y_ppem(FontObject* self, void* closure) {
    return PyLong_FromLong(self->face->size->metrics.y_ppem);
}

static PyObject* font_getattr_glyphs(FontObject* self, void* closure) {
...
static PyTypeObject Font_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "Font", sizeof(FontObject), 0,
    /* methods */
...
static int setup_module(PyObject* m) {
    ...
    if (FT_Init_FreeType(&library)) return 0; /* leave it uninitialized */
...
PyMODINIT_FUNC PyInit__imagingft(void) {
    PyObject* m;
